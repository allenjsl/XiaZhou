...
namespace EyouSoft.BLL.GovStructure
{
    ...
    public class BGood
    {
        ...
        /// <param name="companyId">公司编号</param>
        /// <returns></returns>
        public int DeleteLingYong(string lingYongId, string companyId)
        {
            if (string.IsNullOrEmpty(lingYongId) || string.IsNullOrEmpty(companyId)) return 0;

            int dalRetCode = dal.DeleteLingYong(lingYongId, companyId);

            if (dalRetCode == 1)
            {
                SysStructure.BSysLogHandle.Insert("删除物品领用（或发放）信息：领用（或发放）编号为：" + lingYongId + "。");
            }
