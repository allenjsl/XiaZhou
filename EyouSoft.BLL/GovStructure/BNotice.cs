...
namespace EyouSoft.BLL.GovStructure
{
    ...
    public class BNotice
    {
        ...
        /// <returns></returns>
        public IList<Model.GovStructure.MGovNotice> GetGovNoticeList(string NoticeType, string CompanyId, string Title, string OperatorId, string Operator, int PageSize, int PageIndex, ref int RecordCount)
        {
            IList<Model.GovStructure.MGovNotice> list = null;
            if (!string.IsNullOrEmpty(CompanyId))
            {
                list = new List<Model.GovStructure.MGovNotice>();
                return dal.GetGovNoticeList(NoticeType,CompanyId, Title, OperatorId, Operator, ItemType, PageSize, PageIndex, ref RecordCount);
            }
            return list;
        }
        ...
        public bool AddNoticeReply(Model.GovStructure.MGovNoticeReply model)
        {
            if (model != null && !string.IsNullOrEmpty(model.NoticeId) && !string.IsNullOrEmpty(model.OperatorId))
            {
                ...
                bool result = dal.AddNoticeReply(model);
                if (result)
                {
                    SysStructure.BSysLogHandle.Insert("增加一条回复信息：编号为：" + model.NoticeId);
                }
                return result;
            }
            return false;
        ...
        {
            return dal.GetGovNoticeReplyList(NoticeId, PageSize, PageIndex, ref RecordCount);
        }
        #endregion  成员方法
    }
}
