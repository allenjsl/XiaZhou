...
namespace EyouSoft.BLL.TourStructure
{
    ...
    public class BTour : EyouSoft.BLL.BLLBase
    {
        ...
        /// <returns></returns>
        public IList<EyouSoft.Model.TourStructure.MTourTeamInfo> GetTourTeamList(string CompanyId, int pageSize, int pageIndex, ref int recordCount, EyouSoft.Model.EnumType.TourStructure.ModuleType ModuleType, EyouSoft.Model.TourStructure.MTourTeamSearch TourTeamSearch)
        {
            ...
            return dal.GetTourTeamList(CompanyId, pageSize, pageIndex, ref recordCount, ModuleType, TourTeamSearch, DeptIds, isOnlySeft, LoginUserId);
        }
        ...
        /// <returns></returns>
        public IList<EyouSoft.Model.TourStructure.MTourSanPinInfo> GetTourSanPinList(string CompanyId, int pageSize, int pageIndex, ref int recordCount, EyouSoft.Model.EnumType.TourStructure.ModuleType ModuleType, EyouSoft.Model.TourStructure.MTourSanPinSearch TourSanPinSearch, bool IsShowAll)
        {
            bool isOnlySeft = false;
            int[] DeptIds = null;
            switch (ModuleType)
            ...
            /*if (!TourSanPinSearch.SLDate.HasValue && !TourSanPinSearch.LLDate.HasValue && !TourSanPinSearch.SLDate.HasValue && !TourSanPinSearch.SRDate.HasValue)
            {
                EyouSoft.Model.ComStructure.MComSetting MComSet = new EyouSoft.BLL.ComStructure.BComSetting().GetModel(CompanyId);
                if (MComSet.ShowBeforeMonth != 0)
                {
            ...
            return dal.GetTourSanPinList(CompanyId, pageSize, pageIndex, ref recordCount, ModuleType, TourSanPinSearch, DeptIds, isOnlySeft, LoginUserId);
        }
        ...
        public int AddTourTeam(EyouSoft.Model.TourStructure.MTourTeamInfo info)
        {
            ...
            if ((result == 1 || result == 2) && info.IsJiFen == 1)
            {
                decimal jiFenBiLi = 0;
                ...
                if (jiFenBiLi > 0)
                {
                    ...
                    jiFenInfo.CrmId = info.CompanyInfo.CompanyId;
                    jiFenInfo.IssueTime = System.DateTime.Now;
                    jiFenInfo.JiFen = info.SumPrice * jiFenBiLi;
                    ...
                    jiFenInfo.OrderJinE = info.SumPrice;
        ...
        {
            if (!Verification(info, EyouSoft.Model.EnumType.TourStructure.TourType.组团散拼))
            {
                return false;
            }
            if (!string.IsNullOrEmpty(info.SourceId))
            {
                if (!dal.GetSupplierTourCheckStatus(info.TourId, info.SourceId))
                {
                    if (dal.UpdateTourSanPin(info))
                    {
                        //添加操作日志
                        System.Text.StringBuilder str = new System.Text.StringBuilder();
                        str.AppendFormat("修改{0},计划编号:{1},团号:{2}", info.TourType, info.TourId, info.TourCode);
        ...
        public bool SendTour(EyouSoft.Model.TourStructure.MSendTour SendTour)
        {
            ...
            var temp = GenerateTourNo(SendTour.DeptId, SendTour.CompanyId, SendTour.TourType, SendTour.LDate);

            SendTour.TourCode = temp[0];
            SendTour.XuHao = EyouSoft.Toolkit.Utils.GetInt(temp[1]);
        ...
        {
            bool isOnlySeft;
            int[] DeptIds = GetDataPrivs(EyouSoft.Model.EnumType.PrivsStructure.Menu2.同业分销_收客计划, out isOnlySeft);
        ...
        /// <param name="recordCount"></param>
        /// <returns></returns>
        public IList<EyouSoft.Model.TourStructure.MTourPlanChange> GetTourPlanChange(string CompanyId, int pageSize, int pageIndex, ref int recordCount)
        {
            bool isOnlySeft = false;
            int[] DeptIds = null;
            DeptIds = GetDataPrivs(EyouSoft.Model.EnumType.PrivsStructure.Menu2.计调中心_业务变更, out isOnlySeft);
            return dal.GetTourPlanChange(CompanyId, pageSize, pageIndex, ref recordCount, DeptIds, isOnlySeft, LoginUserId);
        }
        ...
        public IList<EyouSoft.Model.TourStructure.MPlanList> GetZTPlanList(string CompanyId, int pageSize, int pageIndex, ref int recordCount, EyouSoft.Model.TourStructure.MPlanListSearch MPlanListSearch)
        {
            ...
            return dal.GetPlanList(CompanyId, pageSize, pageIndex, ref recordCount, EyouSoft.Model.EnumType.TourStructure.ModuleType.组团, MPlanListSearch, DeptIds, isOnlySeft, LoginUserId);
        }
        ...
        public IList<EyouSoft.Model.TourStructure.MPlanList> GetDJPlanList(string CompanyId, int pageSize, int pageIndex, ref int recordCount, EyouSoft.Model.TourStructure.MPlanListSearch MPlanListSearch)
        {
            ...
            return dal.GetPlanList(CompanyId, pageSize, pageIndex, ref recordCount, EyouSoft.Model.EnumType.TourStructure.ModuleType.地接, MPlanListSearch, DeptIds, isOnlySeft, LoginUserId);
        }
        ...
        public IList<EyouSoft.Model.TourStructure.MPlanList> GetCJPlanList(string CompanyId, int pageSize, int pageIndex, ref int recordCount, EyouSoft.Model.TourStructure.MPlanListSearch MPlanListSearch)
        {
            ...
            return dal.GetPlanList(CompanyId, pageSize, pageIndex, ref recordCount, EyouSoft.Model.EnumType.TourStructure.ModuleType.出境, MPlanListSearch, DeptIds, isOnlySeft, LoginUserId);
        }
        ...
        public IList<EyouSoft.Model.TourStructure.MBZInfo> GetSaleBZList(string CompanyId, int pageSize, int pageIndex, ref int recordCount, EyouSoft.Model.TourStructure.MBZSearch MBZSearch, int DeptId)
        {
            ...
            bool isOnlySelf = false;
            //能查看到该菜单下面数据的部门编号，NULL为所有部门
            int[] DeptIds = GetDataPrivs(EyouSoft.Model.EnumType.PrivsStructure.Menu2.销售中心_销售报账, out  isOnlySelf);
            var setting = new EyouSoft.BLL.ComStructure.BComSetting().GetModel(CompanyId);
        ...
        public IList<EyouSoft.Model.TourStructure.MBZInfo> GetPlanEndList(string CompanyId, int pageSize, int pageIndex, ref int recordCount, EyouSoft.Model.TourStructure.MBZSearch MBZSearch, int DeptId)
        {
            ...
            bool isOnlySelf = false;
            //能查看到该菜单下面数据的部门编号，NULL为所有部门
            int[] DeptIds = GetDataPrivs(EyouSoft.Model.EnumType.PrivsStructure.Menu2.计调中心_计调终审, out  isOnlySelf);
            var setting = new EyouSoft.BLL.ComStructure.BComSetting().GetModel(CompanyId);
        ...
        /// <summary>
        /// 生成团号,返回[0:string:团号 1:string:派团序号]
        /// </summary>
        ...
        public string[] GenerateTourNo(int DeptId, string CompanyId, EyouSoft.Model.EnumType.TourStructure.TourType TourType, DateTime LDate)
        {
            ...
            if (string.IsNullOrEmpty(TourNoSetting))
            {
                ...
                xuHao = dal.GetTourNum(CompanyId, LDate).ToString();
                TourCode += xuHao;
            }
        ...
        {
            if (string.IsNullOrEmpty(tourId)) return null;
            return dal.GetTourStatusXXs(tourId);
        }
    }
}
