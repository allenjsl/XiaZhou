...
namespace EyouSoft.BLL.TourStructure
{
    ...
    public class BTourOrder : EyouSoft.BLL.BLLBase
    {
        ...
        public int UpdateTourOrderExpand(string orderId, OrderStatus orderStatus, MTourOrderChange change)
        {
            ...
            if (!CheckTourOrderChange(change)) return -1;
            ...
            ref int recordCount, out object[] heJi)
        {
            ...
            bool _isOnlySelf;
            int[] deptIds = this.GetDataPrivs(EyouSoft.Model.EnumType.PrivsStructure.Menu2.同业分销_订单中心, out _isOnlySelf);
        ...
        {
            return dal.GetBaoCunDaoYouShouRuStatus(orderId);
        }
        ...
        {
            if (string.IsNullOrEmpty(companyId) || string.IsNullOrEmpty(dingDanId) || string.IsNullOrEmpty(heTongId)) return 0;

            int dalRetCode = dal.SheZhiDingDanHeTong(companyId, dingDanId, heTongId);

            if (dalRetCode == 1)
            {
                EyouSoft.BLL.SysStructure.BSysLogHandle.Insert("设置订单合同，订单编号：" + dingDanId + "，合同编号" + heTongId);
            }
