...
namespace EyouSoft.DAL.CrmStructure
{
    ...
    public class DCrm : EyouSoft.Toolkit.DAL.DALBase, IDAL.CrmStructure.ICrm
    {
        ...
        {
            //<root><info lxrid="编号" deptname="部门" address="地址" lxrname="姓名" telephone="电话" mobile="手机" fax="传真" birthday="生日" istixing="是否生日提醒" qq="qq" /></root>
            StringBuilder xml = new StringBuilder();
            xml.Append("<root>");
            ...
            if (items != null && items.Count > 0)
            {
                foreach (var item in items)
                {
                    ...
                    xml.AppendFormat("<info lxrid=\"{0}\" deptname=\"{1}\" address=\"{2}\" lxrname=\"{3}\" telephone=\"{4}\" mobile=\"{5}\" fax=\"{6}\" birthday=\"{7}\" istixing=\"{8}\" qq=\"{9}\" />", item.Id
                        , Utils.ReplaceXmlSpecialCharacter(item.Department)
                        , Utils.ReplaceXmlSpecialCharacter(item.Address)
        ...
        {
            string sql = "UPDATE tbl_Crm SET PrintHeader = @PrintHeader,PrintFooter = @PrintFooter,PrintTemplates = @PrintTemplates,Seal = @Seal WHERE CrmId =@CrmId";
