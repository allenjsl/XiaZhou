    ...
    public class DArchives : EyouSoft.Toolkit.DAL.DALBase, EyouSoft.IDAL.GovStructure.IArchives
    {
        ...
        public IList<EyouSoft.Model.GovStructure.MGovFile> GetSearchArchivesList(EyouSoft.Model.GovStructure.MSearchGovFile SearchModel, string CompanyId, int PageSize, int PageIndex, ref int RecordCount)
        {
            ...
            if (SearchModel != null)
            {
                ...
                if (!string.IsNullOrEmpty(SearchModel.DangAnId))
                {
                    query += string.Format(" AND Id='{0}' ", SearchModel.DangAnId);
                }
                if (SearchModel.DeptIds != null && SearchModel.DeptIds.Length > 0)
                {
                    query += string.Format(" AND DepartId IN({0}) ", Utils.GetSqlIn<int>(SearchModel.DeptIds));
                }
            }
        ...
        /// <param name="companyId">公司编号</param>
        /// <returns></returns>
        public int GongLingTongBu(string companyId)
        {
            string s = "UPDATE tbl_GovFile SET LengthService=DATEDIFF(YEAR,ISNULL(EntryTime,GETDATE()),GETDATE()) WHERE CompanyId=@CompanyId";
