...
namespace EyouSoft.DAL.GovStructure
{
    ...
    public class DNotice : EyouSoft.Toolkit.DAL.DALBase, EyouSoft.IDAL.GovStructure.INotice
    {
        ...
        {
            bool IsTrue = false;
            if (String.IsNullOrEmpty(model.NoticeType))
                model.NoticeType = "0";
            ...
            this._db.AddInParameter(dc, "NoticeType", DbType.AnsiStringFixedLength, model.NoticeType);
        ...
        /// 增加一条回复
        /// </summary>
        public bool AddNoticeReply(Model.GovStructure.MGovNoticeReply model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into tbl_GovNoticeReply(");
            strSql.Append("ItemId,NoticeId,OperatorId,OperatorName,ReplyInfo,IssueTime)");
            strSql.Append(" values (");
            strSql.Append("@ItemId,@NoticeId,@OperatorId,@OperatorName,@ReplyInfo,@IssueTime)");

            DbCommand dc = this._db.GetSqlStringCommand(strSql.ToString());
            this._db.AddInParameter(dc, "ItemId", DbType.AnsiStringFixedLength, model.ItemId);
            this._db.AddInParameter(dc, "NoticeId", DbType.AnsiStringFixedLength, model.NoticeId);
            this._db.AddInParameter(dc, "OperatorId", DbType.AnsiStringFixedLength, model.OperatorId);
            this._db.AddInParameter(dc, "OperatorName", DbType.String, model.OperatorName);
            this._db.AddInParameter(dc, "ReplyInfo", DbType.String, model.ReplyInfo);
            this._db.AddInParameter(dc, "IssueTime", DbType.DateTime, model.IssueTime);
            return EyouSoft.Toolkit.DAL.DbHelper.ExecuteSql(dc, this._db) > 0 ? true : false;
        }
        /// <summary>
        /// 获得回复信息列表
        /// </summary>
        /// <param name="NoticeId">学习交流编号</param>
        /// <param name="PageSize">页面数据总记录数</param>
        /// <param name="PageIndex">当前页数</param>
        /// <param name="RecordCount">每页显示的记录数</param>
        /// <returns></returns>
        public IList<Model.GovStructure.MGovNoticeReply> GetGovNoticeReplyList(string NoticeId, int PageSize, int PageIndex, ref int RecordCount)
        {
            IList<EyouSoft.Model.GovStructure.MGovNoticeReply> ResultList = null;
            string tableName = "tbl_GovNoticeReply";
            string identityColumnName = "ItemId";
            string fields = "ItemId,NoticeId,OperatorId,OperatorName,ReplyInfo,IssueTime ";
            string query = string.Format(" (NoticeId='{0}')", NoticeId);
            string orderByString = " IssueTime DESC";
            using (IDataReader dr = EyouSoft.Toolkit.DAL.DbHelper.ExecuteReader(_db, PageSize, PageIndex, ref RecordCount, tableName, identityColumnName, fields, query, orderByString))
            {
                ResultList = new List<EyouSoft.Model.GovStructure.MGovNoticeReply>();
                while (dr.Read())
                {
                    EyouSoft.Model.GovStructure.MGovNoticeReply model = new EyouSoft.Model.GovStructure.MGovNoticeReply()
                    {
                        ItemId = dr.GetString(dr.GetOrdinal("ItemId")),
                        NoticeId = dr.GetString(dr.GetOrdinal("NoticeId")),
                        OperatorId = dr.GetString(dr.GetOrdinal("OperatorId")),
                        OperatorName = dr.IsDBNull(dr.GetOrdinal("OperatorName")) ? "" : dr.GetString(dr.GetOrdinal("OperatorName")),
                        ReplyInfo = dr.IsDBNull(dr.GetOrdinal("ReplyInfo")) ? "" : dr.GetString(dr.GetOrdinal("ReplyInfo")),
                        IssueTime = dr.GetDateTime(dr.GetOrdinal("IssueTime"))
                    };
                    ResultList.Add(model);
                    model = null;
