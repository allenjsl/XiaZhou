...
namespace EyouSoft.DAL.IndStructure
{
    ...
    public class DIndividual : DALBase, IIndividual
    {
        ...
        public IList<MOrderRemind> GetOrderRemindLst(int pageSize, int pageIndex, ref int recordCount, string operatorId, string companyId)
        {
            ...
            #region 要查询的字段
            fields.Append(" OrderId,OrderCode,BuyCompanyName,Adults,Childs,SumPrice,TourType ");
            #endregion
            #region 拼接查询条件
            cmdQuery.AppendFormat(" IsDelete=0 and OrderType={0} and CompanyId='{1}' and Status={2}", (int)EyouSoft.Model.EnumType.TourStructure.OrderType.分销商下单, companyId, (int)EyouSoft.Model.EnumType.TourStructure.OrderStatus.未处理);
            cmdQuery.AppendFormat(" and exists(select 1 from tbl_Tour where SellerId='{0}' and TourId=tbl_TourOrder.TourId)", operatorId);
            #endregion
        ...
        public IList<MPlanRemind> GetPlanRemindLst(int pageSize, int pageIndex, ref int recordCount, string companyId, string operatorId)
        {
            ...
            StringBuilder fields = new StringBuilder();
            #region 要查询的字段
            fields.Append(" TourId,TourCode,RouteName,SellerName,LDate,PlanPeopleNumber,(select * from tbl_TourPlanStatus where TourId=tbl_Tour.TourId for xml raw,root('TourPlanStatus')) as TourPlanStatus,TourType ");
            #endregion
            #region 拼接查询条件
            cmdQuery.AppendFormat(" IsDelete=0 and TourStatus={0} and CompanyId='{1}'", (int)EyouSoft.Model.EnumType.TourStructure.TourStatus.计调未接收, companyId);
            cmdQuery.AppendFormat(" and exists(select 1 from tbl_TourPlaner where PlanerId='{0}' and TourId=tbl_Tour.TourId)", operatorId);
            #endregion
            using (IDataReader rdr = DbHelper.ExecuteReader(this._db, pageSize, pageIndex, ref recordCount, TableName, PrimaryKey, fields.ToString(), cmdQuery.ToString(), OrderByString))
            {
                while (rdr.Read())
 ...
 , string companyId, string operatorId)
        {
            ...
            StringBuilder cmdQuery = new StringBuilder();
            string TableName = "view_CustomerWarning";
            string PrimaryKey = "CrmId";
            string OrderByString = "Transfinite DESC";
            StringBuilder fields = new StringBuilder();
            #region 要查询的字段
            fields.Append("  *");
            #endregion
            #region 拼接查询条件
            cmdQuery.AppendFormat(" (Transfinite>0 or DeadDay>0) and SellerId='{0}' and CompanyId='{1}'", operatorId, companyId);
            #endregion
            using (IDataReader rdr = DbHelper.ExecuteReader(this._db, pageSize, pageIndex, ref recordCount, TableName, PrimaryKey, fields.ToString(), cmdQuery.ToString(), OrderByString))
            {
                while (rdr.Read())
                {
                    ...
                    {
                        CrmId = rdr["CrmId"].ToString(),
                        Customer = rdr["Customer"].ToString(),
                        SellerName = rdr["SellerName"].ToString(),
                        AmountOwed = rdr.IsDBNull(rdr.GetOrdinal("AmountOwed")) ? 0 : rdr.GetDecimal(rdr.GetOrdinal("AmountOwed")),
                        Transfinite = rdr.IsDBNull(rdr.GetOrdinal("Transfinite")) ? 0 : rdr.GetDecimal(rdr.GetOrdinal("Transfinite")),
                        Arrear = rdr.IsDBNull(rdr.GetOrdinal("Arrear")) ? 0 : rdr.GetDecimal(rdr.GetOrdinal("Arrear")),
                        TransfiniteTime = rdr.IsDBNull(rdr.GetOrdinal("TransfiniteTime")) ? null : (DateTime?)rdr.GetDateTime(rdr.GetOrdinal("TransfiniteTime")),
                        Deadline = rdr.IsDBNull(rdr.GetOrdinal("Deadline")) ? 0 : rdr.GetInt32(rdr.GetOrdinal("Deadline")),
                        DeadDay = rdr.IsDBNull(rdr.GetOrdinal("DeadDay")) ? 0 : rdr.GetInt32(rdr.GetOrdinal("DeadDay")),
                        TourCount = rdr.GetInt32(rdr.GetOrdinal("TourCount")),
                        Phone = GetValueByXml(rdr["XMLContact"].ToString(), "Telephone"),
                        Contact = GetValueByXml(rdr["XMLContact"].ToString(), "Name")
                    };
                    list.Add(model);
                }
            }
 ...
 , string companyId, string operatorId)
        {
            ...
            StringBuilder fields = new StringBuilder();
            #region 要查询的字段
            fields.Append(" Id,TourId,TourCode,RouteName,SellerName,(select DISTINCT SourceId,SourceName from tbl_Plan where TourId=tbl_TourPlanChange.TourId and Type=12 for xml raw,root) as Guid,(select * from tbl_TourPlaner where TourId=tbl_TourPlanChange.TourId for xml raw,root) as Planer,IssueTime,Operator,Title,[Content] ");
            #endregion
            #region 拼接查询条件
            cmdQuery.AppendFormat(" State='0' and CompanyId='{0}'", companyId);
            cmdQuery.AppendFormat(" and exists(select 1 from tbl_TourPlaner where TourId=tbl_TourPlanChange.TourId and PlanerId='{0}')", operatorId);
            #endregion
            using (IDataReader rdr = DbHelper.ExecuteReader(this._db, pageSize, pageIndex, ref recordCount, TableName, PrimaryKey, fields.ToString(), cmdQuery.ToString(), OrderByString))
            {
                while (rdr.Read())
        ...
        public IList<MLaborContractExpireRemind> GetLaborContractExpireRemindLst(int pageSize, int pageIndex, ref int recordCount, string companyId, int EarlyDays)
        {
            ...
            StringBuilder fields = new StringBuilder();
            #region 要查询的字段
            fields.Append(" (select top 1 FileNumber from tbl_GovFile where ID=tbl_GovFileContract.FileId) as FileNumber,[Name],MaturityTime,ContractNumber,FileId ");
            #endregion
            #region 拼接查询条件
        ...
        public IList<MSourceContractExpireRemind> GetSourceContractExpireRemindLst(int pageSize, int pageIndex, ref int recordCount, string companyId, int EarlyDays)
        {
            ...
            StringBuilder fields = new StringBuilder();
            #region 要查询的字段
            fields.Append(" SourceId,[Name],ContractPeriodEnd,ContractCode,Type ");
            #endregion
            #region 拼接查询条件
            ...
            using (IDataReader rdr = DbHelper.ExecuteReader(this._db, pageSize, pageIndex, ref recordCount, TableName, PrimaryKey, fields.ToString(), cmdQuery.ToString(), OrderByString))
            {
                while (rdr.Read())
                {
                    ...
                    model.GysId = rdr["SourceId"].ToString();
                    model.GysLeiXing = (EyouSoft.Model.EnumType.SourceStructure.SourceType)rdr.GetByte(rdr.GetOrdinal("Type"));
                    list.Add(model);
                }
            }
            return list;
        ...
        /// <param name="recordCount">总记录数</param>
        /// <param name="companyId">系统公司编号</param>
        /// <param name="remindDays">提前提醒的天数</param>
        /// <returns></returns>
        public IList<MBirthdayRemind> GetBirthdayRemindLst(int pageSize, int pageIndex, ref int recordCount, string companyId,int remindDays)
        {
            IList<MBirthdayRemind> list = new List<MBirthdayRemind>();
            MBirthdayRemind model = new MBirthdayRemind();
            StringBuilder cmdQuery = new StringBuilder();
            string TableName = "view_Crm_ContactBirthday";
            string PrimaryKey = "Id";
            string OrderByString = "Id";
            StringBuilder fields = new StringBuilder();
            #region 要查询的字段
            fields.Append(" Id, Type, CompanyId, CompanyName, Name, Gender, Birthday, MobilePhone");
            #endregion
            #region 拼接查询条件
            cmdQuery.AppendFormat(" CompanyId='{0}' and DateAdd(year,Year(Getdate())-Year(Birthday),Birthday) Between Convert(Varchar(10),Getdate(),120) And Convert(Varchar(10),Getdate()+" + remindDays + ",120)", companyId);
            //cmdQuery.AppendFormat("AND  QuoteStatus={0} ", (int)EyouSoft.Model.EnumType.TourStructure.QuoteState.未处理);
            #endregion
            using (IDataReader rdr = DbHelper.ExecuteReader(this._db, pageSize, pageIndex, ref recordCount, TableName, PrimaryKey, fields.ToString(), cmdQuery.ToString(), OrderByString))
            {
                while (rdr.Read())
                {
                    model.Id = !rdr.IsDBNull(rdr.GetOrdinal("Id")) ? (string)rdr["Id"] : string.Empty;
                    model.MobilePhone = !rdr.IsDBNull(rdr.GetOrdinal("MobilePhone")) ? (string)rdr["MobilePhone"] : string.Empty;
                    model.Name = !rdr.IsDBNull(rdr.GetOrdinal("Name")) ? (string)rdr["Name"] : string.Empty;
                    model.Birthday = rdr.IsDBNull(rdr.GetOrdinal("Birthday")) ? null : (DateTime?)(rdr["Birthday"]);
                    model.Gender = (EyouSoft.Model.EnumType.GovStructure.Gender)(int.Parse(rdr["Gender"].ToString()));
                     model.CompanyName = !rdr.IsDBNull(rdr.GetOrdinal("CompanyName")) ? (string)rdr["CompanyName"] : string.Empty;
                     model.Type = (EyouSoft.Model.EnumType.ComStructure.LxrType)(int.Parse(rdr["Type"].ToString()));
        ...
        public int GetRemindCountByType(string CompanyId, string OperatorId, RemindType type,EyouSoft.Model.ComStructure.MComSetting setting)
        {
            ...
            switch (type)
            {
                case RemindType.变更提醒:
                    {
                        ...
                        cmdQuery.AppendFormat("select count(*) as TipCount from tbl_TourPlanChange where State='0' and CompanyId='{0}' and exists(select 1 from tbl_TourPlaner where TourId=tbl_TourPlanChange.TourId and PlanerId='{1}')", CompanyId, OperatorId);
                        cmdQuery.Append(" union all ");
                        cmdQuery.AppendFormat(" select count(*) as TipCount from tbl_TourOrderChange where IsSure='0' and CompanyId='{0}'  and exists(select 1 from tbl_TourPlaner where TourId=tbl_TourOrderChange.TourId and PlanerId='{1}' AND ChangeType={2})) AS a", CompanyId, OperatorId, (int)EyouSoft.Model.EnumType.TourStructure.ChangeType.变更);
                        break;
                    }
                case RemindType.订单提醒:
                    {
                        cmdQuery.AppendFormat("select count(1) as TipCount  from tbl_TourOrder where IsDelete=0 and OrderType={0} and CompanyId='{1}' and Status={2} and exists(select 1 from tbl_Tour where SellerId='{3}' and TourId=tbl_TourOrder.TourId)", (int)EyouSoft.Model.EnumType.TourStructure.OrderType.分销商下单, CompanyId, (int)EyouSoft.Model.EnumType.TourStructure.OrderStatus.未处理, OperatorId);
                        break;
                    }
                case RemindType.长江游轮订单提醒:
                    {
                        cmdQuery.AppendFormat("select count(1) as TipCount from view_YL_HangQiDingDan where IsDelete=0 and CompanyId='{0}' and DingDanStatus={1}  AND LeiXing=0 AND IsTuanGou='0'", CompanyId, (int)EyouSoft.Model.EnumType.YlStructure.HangQiDingDanStatus.未处理);
                        break;
                    }
                case RemindType.海洋游轮订单提醒:
                    {
                        cmdQuery.AppendFormat("select count(1) as TipCount from view_YL_HangQiDingDan where IsDelete=0 and CompanyId='{0}' and DingDanStatus={1}  AND LeiXing=1 AND IsTuanGou='0'", CompanyId, (int)EyouSoft.Model.EnumType.YlStructure.HangQiDingDanStatus.未处理);
                        break;
                    }
                case RemindType.游轮团购订单提醒:
                    {
                        cmdQuery.AppendFormat("select count(1) as TipCount from view_YL_HangQiDingDan where IsDelete=0 and CompanyId='{0}' and DingDanStatus={1}  AND IsTuanGou='1'", CompanyId, (int)EyouSoft.Model.EnumType.YlStructure.HangQiDingDanStatus.未处理);
                        break;
                    }
                case RemindType.合同到期提醒:
                    {
                        ...
                        cmdQuery.Append(" union all ");
                        cmdQuery.AppendFormat("select count(*) as TipCount from tbl_Source where IsDelete='0' AND IsSignContract=1 and CompanyId='{0}' and datediff(day,getdate(),ContractPeriodEnd)<={1}", CompanyId, setting.SContractRemind);
                        cmdQuery.Append(" union all ");
                ...
                case RemindType.计调提醒:
                    {
                        cmdQuery.AppendFormat("select count(*) as TipCount from tbl_Tour where IsDelete='0' and TourStatus={0} and CompanyId='{1}' and exists(select 1 from tbl_TourPlaner where PlanerId='{2}' and TourId=tbl_Tour.TourId)", (int)EyouSoft.Model.EnumType.TourStructure.TourStatus.计调未接收, CompanyId, OperatorId);

                        break;
                    }
                case RemindType.收款提醒:
                    {
                        cmdQuery.AppendFormat("select count(*) as TipCount from view_CustomerWarning where (Transfinite>0 or DeadDay>0) and SellerId='{0}' and CompanyId='{1}'", OperatorId, CompanyId);
                        break;
                    }
                case RemindType.询价提醒:
                    {
                        cmdQuery.AppendFormat("select count(*) as TipCount from tbl_Quote where CompanyId='{0}' and IsPlanerQuote='0' and PlanerId='{1}' AND IsDelete='0'", CompanyId, OperatorId);
                        break;
                    }
                case RemindType.预控到期提醒:
                    {
                        ...
                        cmdQuery.Append(" union all ");
                        cmdQuery.AppendFormat("select count(*) as TipCount from view_SourceSueCar where CompanyId='{0}' AND (isnull(ControlNum,0)-isnull(AlreadyNum,0))>0  AND datediff(day,getdate(),LastTime)<={1}) AS a", CompanyId, setting.CarControlRemind);/*
                        cmdQuery.Append(" union all ");
                        cmdQuery.AppendFormat("select count(*) as TipCount from tbl_CrmLinkman where CompanyId='{0}' AND (isnull(ControlNum,0)-isnull(AlreadyNum,0))>0  AND datediff(day,getdate(),LastTime)<={1}) AS a", CompanyId, setting.ShipControlRemind);*/
                        break;
                    }
                case RemindType.生日提醒:
                    {
                        cmdQuery.AppendFormat("select count(1) as TipCount FROM tbl_CrmLinkman WHERE CompanyId='{0}' AND (MONTH(Birthday) = MONTH(GETDATE())) AND (DAY(Birthday) = DAY(GETDATE()))", CompanyId);
                        break;
                    }
            }
