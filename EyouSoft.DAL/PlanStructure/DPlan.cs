...
namespace EyouSoft.DAL.PlanStructure
{
    ...
    public class DPlan : EyouSoft.Toolkit.DAL.DALBase, EyouSoft.IDAL.PlanStructure.IPlan
    {
        ...
        public IList<MPlanBaseInfo> GetList(string tourId)
        {
            ...
            sql.Append("         [CostDetail] ,");
            sql.Append("         [PaymentType] ,");
            sql.Append("         [Confirmation],PlanId,[DNum],Prepaid ");
            sql.Append(" FROM    [dbo].[tbl_Plan]");
        ...
        public IList<MPlan> GetList(PlanProject planType, Payment? payment, PlanAddStatus? addStatus, bool isShowCostChange, PlanChangeChangeClass? changeType, string tourId, PlanState? anPaiStatus)
        {
            ...
            {
                sql.Append("         XMLTrainBus = ( SELECT    ISNULL(SUM(T.FreeNumber),0)FreeNumber,ISNULL(SUM(T.PayNumber),0)PayNumber,CONVERT(VARCHAR(19),MIN(T.DepartureTime),120) DepartureTime");
            ...
            if (anPaiStatus.HasValue)
            {
                sql.AppendFormat(" AND P.Status={0} ", (int)anPaiStatus.Value);
            }
            ...
            if (mdl != null)
            {
                ...
                if (!string.IsNullOrEmpty(xml))
                {
                    ...
                    foreach (var xrow in xrows)
                    {
                        ...
                        string s = Utils.GetXAttributeValue(xrow, "C");
            ...
            using (var dr = DbHelper.ExecuteReader(dc, this.db))
            {
                while (dr.Read())
                {
                    lst.Add(new MOtherFeeInOut
                    {
                        ...
                        OperatorId = dr["OperatorId"].ToString(),
                        IsGuide = (PlanAddStatus)dr.GetByte(dr.GetOrdinal("IsGuide")),
                    });
                }
            }
            return lst;
        ...
        public MBZHZ GetBZHZ(string tourid)
        {
            ...
            using (var dr = DbHelper.ExecuteReader(dc, this.db))
            {
                while (dr.Read())
                {
                    ...
                    mdl.GuideOutlay = this.GetPlanCost(Payment.导游现付, tourid);
                    mdl.GuideRelSign = Utils.GetInt(Utils.GetValueFromXmlByAttribute(dr["XMLGuide"].ToString(), "RelSignNum"));
                    mdl.GuideUsed = this.GetPlanCount(Payment.导游签单, tourid);
                }
            }
            return mdl;
        }
        ...
        public MTourTotalInOut GetTourTotalInOut(string tourId)
        {
            ...
            sql.Append(",QiTaShouRu=(SELECT ISNULL(SUM(A1.FeeAmount),0) FROM tbl_FinOtherInFee AS A1 WHERE A1.TourId=@TourId AND A1.IsDeleted='0')");
            sql.Append(",DaoYouZhiFuJinE=(SELECT ISNULL(SUM(Confirmation),0) FROM tbl_Plan WHERE TourId=tbl_Tour.TourId AND Status=@PlanStatus AND IsDelete='0' AND PaymentType IN(@ZhiFuFangShi1,@ZhiFuFangShi2)) ");
            ...
            db.AddInParameter(dc, "@ZhiFuFangShi1", DbType.Byte, Payment.导游签单);
            db.AddInParameter(dc, "@ZhiFuFangShi2", DbType.Byte, Payment.导游现付);
            ...
            using (var dr = DbHelper.ExecuteReader(dc, this.db))
            {
                while (dr.Read())
                {
                    ...
                    mdl.QiTaShouRu = dr.GetDecimal(dr.GetOrdinal("QiTaShouRu"));
                    mdl.DaoYouZhiFuJinE = dr.GetDecimal(dr.GetOrdinal("DaoYouZhiFuJinE"));                 
                }
            }
