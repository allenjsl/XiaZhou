...
namespace EyouSoft.DAL.StatStructure
{
    ...
    public class DStatistics : DALBase, IStatistics
    {
        ...
        int GetDeptRenShu(int deptId,string s)
        {
            string sql = "SELECT COUNT(DISTINCT (SellerId)) PeopleNum FROM view_SellerYeJi WHERE DeptId=@DeptId ";
        ...
        int GetDeptRenShu1(int deptId)
        {
            string sql = "SELECT COUNT(*) PeopleNum FROM tbl_ComUser WHERE DeptId=@DeptId AND IsDelete='0' ";
            ...
            , MDepartmentSearch search, ref MDepartmentTongJi TongJi)
        {
            ...
            StringBuilder fields = new StringBuilder();
            #region 要查询的字段
            fields.Append(" DeptId,sum(OrderNum) OrderNum,/*count(distinct (SellerId))*/0 AS PeopleNum,sum(PeopleNum) OrderPersonNum,sum(InCome) InCome,sum(Pay) Pay, sum(GrossProfit) GrossProfit ");
            fields.Append(
            ...
            if (search != null)
            {
                ...
                {
                    //cmdQuery.AppendFormat(" and datediff(day,'{0}',ReviewTime)>=0", search.SReviewTime);
                    cmdQuery.AppendFormat(" AND ReviewTime>='{0}' ", search.SReviewTime.Value);
                }

                if (search.EReviewTime.HasValue)
                {
                    //cmdQuery.AppendFormat(" and datediff(day,'{0}',ReviewTime)<=0", search.EReviewTime);
                    cmdQuery.AppendFormat(" AND ReviewTime<='{0}' ", search.EReviewTime.Value);
                }
                if (!search.SReviewTime.HasValue && !search.EReviewTime.HasValue)
                {
                    //cmdQuery.Append(" and datediff(month,ReviewTime,getdate())=0");
                    cmdQuery.AppendFormat (" AND ReviewTime>='{0}' AND ReviewTime<='{0}'", DateTime.Today);
                }
            }
            else
            {
            ...
            if (list != null && list.Count > 0)
            {
                ...
                {
                    //item1.PeopleNum = GetDeptRenShu(item1.DeptId, cmdQuery.ToString());
                    //item1.PeopleNum = GetDeptRenShu1(item1.DeptId);
                }
            }
            ...
            , string deptIds, MPersonalSearch search, ref MPersonalOrderListTongJi tongJi)
        {
            ...
            using (IDataReader rdr = DbHelper.ExecuteReader(this._db, pageSize, pageIndex, ref recordCount, ref  SumMoney, TableName, XMLSumField, fields.ToString(), cmdQuery.ToString(), OrderByString))
            {
                while (rdr.Read())
                {
                    ...
                    if (!rdr.IsDBNull(rdr.GetOrdinal("LDate"))) item.LDate = rdr.GetDateTime(rdr.GetOrdinal("LDate"));
        ...
        public IList<MReconciliation> GetReconciliationLst(string companyId, int pageSize, int pageIndex, ref int recordCount, string deptIds, MReconciliationSearch mSearch, ref MReconciliationTongJi TongJi)
        {
            ...
            if (mSearch != null)
            {
                ...
                if (mSearch.RestAmount.HasValue&&mSearch.EqualSign.HasValue)
                {
                    string _operator = Utils.GetSqlBiJiaoYunSuanFu((int)mSearch.EqualSign.Value);
                    cmdQuery.AppendFormat(" and NotIncome{0}{1}", _operator, mSearch.RestAmount.Value);
                }
        ...
        public IList<MTourStatus> GetTourStatusLst(string companyId, int pageSize, int pageIndex, ref int recordCount, int[] deptIds, MTourStatusSearch mSearch, out object[] heJi)
        {
            ...
            string OrderByString = "LDate DESC";
            StringBuilder fields = new StringBuilder();
            string heJiString = "SUM(ShiShouRenShu) AS RenShuHeJi";

            #region 要查询的字段
            fields.Append(" *,(select * from tbl_TourPlaner where TourId=view_TongJi_YuSuanBiao.TourId for xml raw,root) as PlanerList,(select SourceId,SourceName from tbl_Plan where TourId=view_TongJi_YuSuanBiao.TourId and IsDelete='0' and [Type]=12 for xml raw,root) as GuideList ");
            #endregion
            #region 拼接查询条件
            cmdQuery.AppendFormat(" CompanyId='{0}' ", companyId);
            ...
            if (mSearch != null)
            {
                ...
                {
                    cmdQuery.AppendFormat(
                        " and exists(select 1 from tbl_Plan where tbl_Plan.TourId = view_TongJi_YuSuanBiao.TourId and tbl_Plan.IsDelete = '0' and tbl_Plan.Type = {0} ",
                        (int)PlanProject.导游);
                ...
                {
                    cmdQuery.AppendFormat(" AND EXISTS(SELECT 1 FROM tbl_TourPlaner AS A WHERE A.TourId=view_TongJi_YuSuanBiao.TourId AND A.PlanerId='{0}') ", mSearch.JiDiaoYuanId);
                }
                else if (!string.IsNullOrEmpty(mSearch.JiDiaoYuanName))
                {
                    cmdQuery.AppendFormat(" AND EXISTS(SELECT 1 FROM tbl_TourPlaner AS A WHERE A.TourId=view_TongJi_YuSuanBiao.TourId AND A.Planer LIKE '%{0}%') ", mSearch.JiDiaoYuanName);
                }
            }
            ...
            using (IDataReader rdr = DbHelper.ExecuteReader1(_db, pageSize, pageIndex, ref recordCount, TableName, fields.ToString(), cmdQuery.ToString(), OrderByString, heJiString))
            {
                while (rdr.Read())
                {
                    item = new EyouSoft.Model.StatStructure.MTourStatus()
                    {
                        ...
                        TourCode = rdr["TourCode"].ToString(),
                        LDate = rdr.IsDBNull(rdr.GetOrdinal("LDate")) ? System.DateTime.MinValue : rdr.GetDateTime(rdr.GetOrdinal("LDate")),
                        PersonNum = rdr.IsDBNull(rdr.GetOrdinal("ShiShouRenShu")) ? 0 : rdr.GetInt32(rdr.GetOrdinal("ShiShouRenShu")),
