...
namespace EyouSoft.DAL.TourStructure
{
    ...
    public class DQuote : EyouSoft.Toolkit.DAL.DALBase, EyouSoft.IDAL.TourStructure.IQuote
    {
        ...
        /// <returns></returns>
        public IList<EyouSoft.Model.TourStructure.MTourQuoteInfo> GetTourQuoteList(string CompanyId, int pageSize, int pageIndex, ref int recordCount, EyouSoft.Model.TourStructure.MTourQuoteSearch info, EyouSoft.Model.EnumType.TourStructure.ModuleType ModuleType, int[] DeptId, bool isOnlySelf, string LoginUserId, int ShowBeforeMonth, int ShowAfterMonth)
        {
            ...
            #endregion
            #region 拼接查询条件
            cmdQuery.AppendFormat(" IsLatest='1' and IsDelete='0' and QuoteType={0} and CompanyId='{1}'", (int)ModuleType, CompanyId);
            if (isOnlySelf)
            {
        ...
        {
            //<Root><Traveller TravellerId="游客编号" OrderId="订单编号" CnName="游客中文姓名" EnName="游客英文姓名" VisitorType="游客类型" CardType="证件类型" CardNumber="证件号码" CardValidDate="证件有效期" VisaStatus="签证状态" IsCardTransact="证件是否已办理" Gender="性别" Contact="联系方式" LNotice="出团通知" RNotice="回团通知" Remark="备注" Status="游客状态" CareId="身份证号" IsInsurance="是否有保险"/></Root>
            if (list == null || list.Count == 0) return null;
            StringBuilder xmlDoc = new StringBuilder();
            xmlDoc.Append("<Root>");
            foreach (var item in list)
            {
                ...
                xmlDoc.AppendFormat("<Traveller TravellerId=\"{0}\" OrderId=\"{1}\" CnName=\"{2}\" EnName=\"{3}\" VisitorType=\"{4}\" CardType=\"{5}\" CardNumber=\"{6}\" CardValidDate=\"{7}\" VisaStatus=\"{8}\" IsCardTransact=\"{9}\" Gender=\"{10}\" Contact=\"{11}\" LNotice=\"{12}\" RNotice=\"{13}\" Remark=\"{14}\" Status=\"{15}\" CardId=\"{16}\" IsInsurance=\"{17}\"/>", item.TravellerId, OrderId, Utils.ReplaceXmlSpecialCharacter(item.CnName), Utils.ReplaceXmlSpecialCharacter(item.EnName), (int)item.VisitorType, (int)item.CardType, item.CardNumber, item.CardValidDate, (int?)(item.VisaStatus.HasValue ? item.VisaStatus : null), item.IsCardTransact ? "1" : "0", (int)item.Gender, Utils.ReplaceXmlSpecialCharacter(item.Contact), item.LNotice ? "1" : "0", item.RNotice ? "1" : "0", Utils.ReplaceXmlSpecialCharacter(item.Remark), (int)EyouSoft.Model.EnumType.TourStructure.TravellerStatus.在团, item.CardId, item.IsInsurance ? "1" : "0");
            }
            xmlDoc.Append("</Root>");
            return xmlDoc.ToString();
        }
