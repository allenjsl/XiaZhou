...
namespace EyouSoft.Security.Membership
{
    ...
    internal class DUserLogin : DALBase, IUserLogin
    {
        ...
        const string SQL_SELECT_GetComMenus = "SELECT A.[Id],A.[Name],(SELECT B.[Id],B.[Name],B.[Privs2Id],B.[Url],B.[Privs] FROM [tbl_SysMenu] AS B WHERE B.[ParentId]=A.[Id] FOR XML RAW,ROOT('root')) AS Menu2,A.[ClassName],A.[IsDisplay]  FROM [tbl_SysMenu] AS A WHERE A.[Privs2Id]=0 AND A.[SysId]=@SysId";
