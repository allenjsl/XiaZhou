            ...
            <asp:Repeater ID="rpt_list" runat="server">
                <ItemTemplate>
                    <td align="left" style="padding: 5px 5px; width: 25%;">
                        <label>
                            <input type="checkbox" name="contactID" value="<%#Eval("CrmId")%>" data-kehudengjibh="<%#Eval("KeHuDengJiBH") %>"
                                data-type="<%=(int)type %>" <%#GetAttrStr(Eval("Lxrs")) %> />
                            <input type="radio" name="contactID" value="<%#Eval("CrmId")%>" data-kehudengjibh="<%#Eval("KeHuDengJiBH") %>"
                                data-type="<%=(int)type %>" <%#GetAttrStr(Eval("Lxrs")) %> />
                ...
                if ((parseInt(Boxy.queryString("IsApply")) || 0) == 1) {
                    ...
                    $("#tab_list :checkbox").each(function() {
                        ...
                        obj.closest("td").find("span[data-class='sp_ContactName']").html((function() {
                            //联系人编号
                            var contactids = obj.attr("data-contactid").split("|").reverse();
                            //联系人名字
                            var contactnames = obj.attr("data-contactname").split("|").reverse();
                            //联系人手机
                            var mobilephones = obj.attr("data-mobilephone").split("|").reverse();
                            //联系人电话
                            var contactphones = obj.attr("data-contactphone").split("|").reverse();
                            //联系人部门
                            var department = obj.attr("data-department").split("|").reverse();
                            //客户等级
                            ...
                            strArr.push("<th align=center  width=7%>选择</th><th align=center  width=7%>编号</th><th align=center width=19%>联系人</th><th align=center width=15%>部门</th><th align=center width=20%>电话</th><th align=center width=20%>手机</th>");
                            strArr.push("</tr>");
                            //内容
                            if (obj.attr("data-contactid").length <= 0) {
                                ...
                                while (i--) {
                                    ...
                                    strArr.push("<td align=center>" + (contactids.length - i) + "</td>");
                                    strArr.push("<td align=center>" + contactnames[i] + "</td>");
                                    strArr.push("<td align=center>" + department[i] + "</td>");
                                    strArr.push("<td align=center>" + contactphones[i] + "</td>");
                                    strArr.push("<td align=center>" + mobilephones[i] + "</td>");
                                    strArr.push("</tr>");
                                }
                            }
                            strArr.push("</table>");
