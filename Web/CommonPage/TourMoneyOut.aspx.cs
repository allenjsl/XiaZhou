...
namespace Web.CommonPage
{
    ...
    public partial class TourMoneyOut : BackPage
    {
        ...
        {
            Payment p = (Payment)pType;
            if (IsShowOperate == -1 || (p == Payment.导游现付 && IsChangeDaoYou == false))
            {
                return p.ToString();
            }
        ...
        protected string GetOperate(int addStatus, int payType)
        {
            ...
            else
            {
                ...
                string isShow = IsChangeDaoYou == false && (Payment)payType == Payment.导游现付 ? "none" : "";
        ...
        /// <param name="p"></param>
        private bool Bind(Repeater rpt, IList<MPlan> items, Panel p)
        {
            if (items != null && items.Count > 0)
            {
