...
namespace Web.CustomerCenter.AjaxRequest
{
    ...
    /// 时间 :2011-10-1
    /// </summary>
    public partial class AjaxTrading : BackPage
    {
        ...
        private void BindData(string crmId, string type)
        {
            ...
            IList<EyouSoft.Model.CrmStructure.MTradingRecord> list = crmBll.GetTradingRecordList(base.SiteUserInfo.CompanyId, crmId, pageIndex, pageSize, ref recordCount);
            if (list != null && list.Count > 0)
            {
                rptJYJLList.DataSource = list;
                rptJYJLList.DataBind();
                BindPage(type);
            }
            else
            {
                this.rptJYJLList.Controls.Add(new Label() { Text = "<tr><td colspan='6' align='center'>未找到相关数据!</td></tr>" });
