<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/Front.Master" AutoEventWireup="true"
    CodeBehind="IncomeList.aspx.cs" Inherits="Web.FinanceManage.IncidentalExpenses.IncomeList" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <div class="mainbox">
        <form id="SelectFrom" action="IncomeList.aspx" method="get">
                        ...
                        <tr data-otherfeeid="<%#Eval("Id") %>" data-intstatus="<%#(int)Eval("Status") %>"
                            ...
                            <td align="center">
                                <%if (this.PrivsPage[0] == "1")
                                  { %>
                                <a data-class="a_ExamineV" data-intstatus="<%#(int)Eval("Status") %>" href="javascript:void(0);"><%#this.GetFinIn(Eval("Id")) %>
                                </a>
                                <%} %>
                                <%if (this.PrivsPage[1] == "1")
                                  { %>
    ...
    <script type="text/javascript">
        var IncomeList = {
            ...
            DataBoxy: function() {/*弹窗默认参数*/
                return {
                    url: "/FinanceManage/IncidentalExpenses",
                    title: "杂费收入",
                    width: "650px",
                    height: "230px"
                }
            ...
            DelAll: function(arr) {/*删除*/
                var Ids = [], url = "/FinanceManage/IncidentalExpenses/IncomeList.aspx?" + $.param({
                    sl: '<%=EyouSoft.Common.Utils.GetQueryStringValue("sl") %>',
                    doType: 'del'
                });
                $(arr).each(function() {
            ...
            BindBtn: function() {/*绑定功能按钮*/
                ...
                $("#li_Add").click(function() {
                    ...
                    data.url += ("/Add.aspx?" + $.param({ sl: '<%=EyouSoft.Common.Utils.GetQueryStringValue("sl") %>', parent: 1 }));
                    that.ShowBoxy(data);
                    return false;
                })
                $("a[data-class='a_openInvoice']").click(function() {/*开票*/
                    var data = that.DataBoxy();
                    var tr = $(this).closest("tr");
                    data.title += EnglishToChanges.Ping("OpenInvoice");
                    data.url = "/FinanceManage/Common/OpenInvoice.aspx?" + $.param({
                        sl: '<%=EyouSoft.Common.Utils.GetQueryStringValue("sl") %>',
                        ParentType: 1,
                        TourId: tr.attr("data-tourid"),
                        CustomerId: tr.attr("data-crmid"),
                        Customer: tr.attr("data-crm"),
                        SellerId: tr.attr("data-sellerId"),
                        SellerName: tr.attr("data-sellerName"),
                        Contact: tr.attr("data-contact"),
                        Phone: tr.attr("data-phone"),
                        Id: tr.attr("data-otherfeeid")
                    });
                    data.height = "400px";
                    data.width = "800px";
                    that.ShowBoxy(data);
