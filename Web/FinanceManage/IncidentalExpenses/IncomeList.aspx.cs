...
namespace Web.FinanceManage.IncidentalExpenses
{
    ...
    /// 创建人：柴逸宁
    /// 创建时间：2011-9-8
    public partial class IncomeList : BackPage
    {
        /// <summary>
        ...
        {
            if (!CheckGrant(Privs.财务管理_杂费收入_栏目))
            {
            ...
            {
                pan_Add.Visible = pan_copy.Visible = CheckGrant(Privs.财务管理_杂费收入_登记);

                pan_update.Visible = CheckGrant(Privs.财务管理_杂费收入_修改);

                pan_delete.Visible = CheckGrant(Privs.财务管理_杂费收入_删除);

                pan_shenhe.Visible = CheckGrant(Privs.财务管理_杂费收入_审核);
                PrivsPage[0] = CheckGrant(Privs.财务管理_杂费收入_审核) ? "1" : "0";
                PrivsPage[1] = CheckGrant(Privs.财务管理_杂费收入_开票登记) ? "1" : "0";
            }
        }
        ...
        /// 获取财务入账
        /// </summary>
        /// <param name="id">杂费收入登记编号</param>
        /// <returns></returns>
        protected string GetFinIn(object id)
        {
            return new BFinance().IsFinIn(DefaultProofType.杂费收入, id.ToString(), this.SiteUserInfo.CompanyId)
                       ? "已入帐 "
                       : "未入账 ";
        }
        #endregion
