...
namespace Web.FinanceManage.IncidentalExpenses
{
    ...
    /// 创建人：柴逸宁
    /// 创建时间：2011-9-8
    public partial class PayList : BackPage
    {
        ...
        {
            if (!CheckGrant(Privs.财务管理_杂费支出_栏目))
            {
                Utils.ResponseNoPermit(Privs.财务管理_杂费支出_栏目, true);
                return;
            }
            else
            {
                ...
                pan_zhifu.Visible = CheckGrant(Privs.财务管理_杂费支出_支付);
            }
        }
        ...
        /// 获取财务入账
        /// </summary>
        /// <param name="id">杂费支出登记编号</param>
        /// <returns></returns>
        protected string GetFinIn(object id)
        {
            return new BFinance().IsFinIn(DefaultProofType.杂费支出, id.ToString(), this.SiteUserInfo.CompanyId)
                       ? "已入帐 "
                       : "未入账 ";
        }
        #endregion
