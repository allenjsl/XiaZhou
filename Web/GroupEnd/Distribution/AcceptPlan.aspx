<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="AcceptPlan.aspx.cs" Inherits="Web.GroupEnd.Distribution.AcceptPlan" %>
...
<body style="background: 0 none;">
    ...
    <div class="list-main">
        <div class="linebox-menu" id="AreaDiv">
            <asp:Repeater runat="server" ID="rpArea">
                <ItemTemplate>
                    <a data-id="<%#Eval("AreaId") %>" title="<%#Eval("AreaName") %>" href="AcceptPlan.aspx?sl=<%=EyouSoft.Common.Utils.GetQueryStringValue("sl") %>&AreaId=<%#Eval("AreaId") %>">
                        <%#Eval("AreaName").ToString().Length > 5 ? GetAreaName(Eval("AreaName").ToString()) : Eval("AreaName")%></a>
                </ItemTemplate>
            </asp:Repeater>
        </div>
                            ...
                            <tr class="<%#Container.ItemIndex%2==0?"":"odd" %>">
                                ...
                                <td align="left">
                                    <a href="<%#PrintPageSp %>?tourId=<%#Eval("TourId") %>" class="lineInfo" target="_blank">
                                        <%#Eval("RouteName")%><font style="color: Red"><%#GetTourType(Eval("TourType")) %></font></a>
                                </td>
                                <td align="center">
                                ...
                                <td align="right" class="font-orange">
                                    <%#EyouSoft.Common.UtilsCommons.GetMoneyString(Eval("SaleAdultPrice"),this.ProviderToMoney)%>
                                </td>
                                <td align="center">
    ...
    <script type="text/javascript">
        var AcceptPlan = {
            ...
            SetArea: function() {
                $("#AreaDiv").find("a").each(function() {
                    if ($(this).attr("data-id") == AcceptPlan.Data.AreaId) {
                    ...
                    if (data) {
                        ...
                        $(this).bind("click", function() {
                            if (data != shoukestatus) {
                                ...
                                var url = "/GroupEnd/Distribution/AcceptPlanApply.aspx?sl=" + AcceptPlan.Data.sl + "&TourId=" + $(this).attr("data-tourid");
                                if (Tourtype == "组团散拼短线") {
                                    url += "&IsShort=1";
                                }
