<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Login.aspx.cs" Inherits="Web.GroupEnd.Distribution.Login" %>
                    ...
                    <ul>
                        ...
                        <li>验证码：<input type="text" class="loginInputText yzm" id="vc" name="" />
                            <img style="cursor: pointer; margin-top: 4px" title="点击更换验证码" onclick="this.src='/ashx/ValidateCode.ashx?ValidateCodeName=gycode&id='+Math.random();return false;"
                                align="middle" width="60" height="20" id="validateImg" src="/ashx/ValidateCode.ashx?ValidateCodeName=DCode&t=<%=DateTime.Now.ToString("HHmmssffff") %>" />
                        </li>
        ...
        function login() {
            ...
            $(".loginbtn").unbind().css("cursor", "default");
             ...
             blogin5($("form").get(0), function(status) {
                 tableToolbar._showMsg("登录成功，正进入分销商平台....");
                 var url = "<%=Request.QueryString["returnurl"] %>";
                 if(url==""){
                    if(status=="1"){
                        url="/GroupEnd/Distribution/AcceptPlan.aspx?sl=<%=(int)EyouSoft.Model.EnumType.PrivsStructure.Menu2.None%>";
                    }
                 }
