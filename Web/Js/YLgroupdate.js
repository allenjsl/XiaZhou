...
var QGD = {
    ...
    html: '<table width="990" border="0" align="center" cellpadding="0" cellspacing="0" style="margin:0 auto;"><tbody>' +
  '<tr>' +
  '<td width="495" height="30" align="center">&nbsp;<a href="javascript:void(0);" id="linkPreMonth"><img src="../images/rili_l.gif" width="14" height="14" />上一月</a></td>' +
  '<td width="495" height="30" align="center">&nbsp;<a href="javascript:void(0);" id="linkNextMonth"><img src="../images/rili_r.gif" width="14" height="14" />下一月</a></td>' +
  '</tr>' +
  '<tr>' +
...
' </table>',
    ...
    initCalendar: function(option) {
        ...
        //		{
        this.maxDate.setMonth(this.maxDate.getMonth() + 23);
        //		}else{
        //			this.maxDate.setMonth(this.maxDate.getMonth()+1);
        //		}

        this.minDate = copyDate(this.option.currentDate);
        this.minDate = formatDate(this.minDate);
        ...
        this.config.arrOldLeaveDate = this.option.arrOldLeaveDate != "" ? this.option.arrOldLeaveDate.split(',') : "";
        ...
        if (this.config.arrOldLeaveDate != undefined && this.config.arrOldLeaveDate.length > 0) {
            //  this.createChildrenTourHTML(this.config.arrOldLeaveDate, "");
        }
        /*
        //初始化基础数据
        this.initBasic();
    ...
    _createCalendarContainer: function() {
        ...
        if (result == -1 || result == 0) {
            ...
            $("#linkPreMonth").css("display", "").unbind().click(function() {
                ...
                pageDataJS.callBack(); //调用初始化页面样式的方法
            });
        }
    ...
    createCalendarMonth: function(isNextMonth) {
        ...
        var s = [];
        //>
        s.push('<table class="rili_box" width="490" border="0" bgcolor="#FFFFFF" align="center" cellspacing="0" cellpadding="0" id="' + tableId + '"><tbody>');
        //s.push('<tr><th style="height:30px;" colspan="7"><input type="checkbox" id="' + isNextMonth + 'ckAll">' + (isNextMonth ? this.SDConfig.NY : this.SDConfig.CY) + '年' + (isNextMonth ? this.SDConfig.NM : this.SDConfig.CM) + '月</th></tr>');
        s.push('<tr><th style="height:30px;" colspan="7">' + (isNextMonth ? this.SDConfig.NY : this.SDConfig.CY) + '年' + (isNextMonth ? this.SDConfig.NM : this.SDConfig.CM) + '月</th></tr>');
        s.push('<tr bgcolor="#F5F4CE">');
        s.push('<th width="70" bgcolor="#F5F4CE" align="center"  id="' + isNextMonth + '0">星期日</th>');
        s.push('<th width="70" bgcolor="#F5F4CE" align="center"  id="' + isNextMonth + '1">星期一</th>');
        s.push('<th width="70" bgcolor="#F5F4CE" align="center"  id="' + isNextMonth + '2">星期二</th>');
        s.push('<th width="70" bgcolor="#F5F4CE" align="center"  id="' + isNextMonth + '3">星期三</th>');
        s.push('<th width="70" bgcolor="#F5F4CE" align="center"  id="' + isNextMonth + '4">星期四</th>');
        s.push('<th width="70" bgcolor="#F5F4CE" align="center"  id="' + isNextMonth + '5">星期五</th>');
        s.push('<th width="70" bgcolor="#F5F4CE" align="center"  id="' + isNextMonth + '6">星期六</th></tr>');
        s.push('</tbody></table>');
    ...
    createEmptyRows: function() {
        var s = '<tr style="height:21px;"><td>&nbsp;</td><td>&nbsp;</td><td></td><td></td><td></td><td></td><td></td></tr>';
    ...
    createCalendarDays: function(isNextMonth) {
        ...
        do {
            ...
            //s.push('<td class="Xdays "><input type="checkbox"' + (sd < this.minDate.getDate() && isCurrentMonth ? ' disabled="disabled"' : '') + ' value="' + tmpDate + '"><span>' + sd + '</span><br/><span class="price"><a class="YuKong"  id="' + tmpDate + '" href="javascript:;">增加</a><span></span></span></td>');
            s.push('<td class="Xdays"><span>' + sd + '</span><br/><span><input type="checkbox"' + (sd < this.minDate.getDate() && isCurrentMonth ? ' disabled="disabled"' : '') + ' value="' + tmpDate + '"></span></td>');
        ...
        //天选中绑定事件
        //obj.find('td.Xdays input[type=checkbox]').bind("click", function() { myself.selectDay(this, isNextMonth); });
    ...
    mouseoutColumn: function(obj) {
        $(obj).closest("table").find("tr:gt(1)").each(function() { $(this).find("td:eq(" + $(obj).parent().find("td").index($(obj)) + ")").css({ background: '' }) });
    },
    //onmouseover 第N周 行
    ...
    mouseoutRow: function(obj) {
        $(obj).parent().children("td").css({ background: '' });
    },
    //选中子团的日期
    initCalendarChecked: function() {
        ...
        for (var i = 0; i < tmp.length; i++) {
            ...
            if (m == this.SDConfig.CM) {
                this.config.JCO.find('td.Xdays input[type="checkbox"]:eq(' + (d - 1) + ')').attr("checked", true);
            } else {
                this.config.JNO.find('td.Xdays input[type="checkbox"]:eq(' + (d - 1) + ')').attr("checked", true);
            }
        }
    },
    //获取星期
    ...
    GetCheckedDate: function() {
        ...
        for (var i = 0; i < objs.length; i++) {
            objs[i].obj.find('td.Xdays input[type="checkbox"]:enabled').each(function() {
    ...
    InitCheckedDate: function() {
        ...
        if (QGD.config.arrLeaveDate.length > 0) {
            for (var i = 0; i < objs.length; i++) {
                objs[i].obj.find('td.Xdays input[type="checkbox"]').each(function() {
                    var tmpDate = $(this).val();
                    for (var h = 0; h < QGD.config.arrLeaveDate.length; h++) {
                        if (tmpDate == QGD.config.arrLeaveDate[h]) {
        ...
        if (this.config.arrOldLeaveDate != '' && this.config.arrOldLeaveDate != null) {
            ...
            for (var j = 0; j < objs.length; j++) {
                for (var i = 0; i < this.config.arrOldLeaveDate.length; i++) {
                    ...
                    if (m == objs[j].month) {
                        var day = d - 1;
                        objs[j].obj.find('td.Xdays input[type=checkbox]:eq(' + day + ')').attr("checked", true);
                        objs[j].obj.find('td.Xdays input[type=checkbox]:eq(' + day + ')').closest("a").html("取消");
