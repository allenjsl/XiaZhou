...
Boxy.prototype = {
    ...
    _setupTitleBar: function() {
        if (this.options.title) {
            var self = this;
            var tb = jQuery("<div class='title-bar' ondblclick=\"Boxy.prototype._ondblclick('" + this.options.iframeId + "','" + this.options.width + "','" + this.options.height + "');\"></div>").html("<h2>" + this.options.title + "</h2>");
            if (this.options.closeable) {
    ...
    _ondblclick: function(iframeId, width, height) {
        var o = document.getElementById(iframeId);
        if ($(o).attr("max") != "1") {
            //if(o.height==height){
            o.width = $(window).width();
            o.height = $(window).height() - 30;
            Boxy.get(o).moveTo(0, 0);
            $(o).attr("max", "1");
        } else {
            o.width = width;
            o.height = height;
