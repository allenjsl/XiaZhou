...
namespace Web.ManageCenter.Notice
{
    /// <summary>
    /// 行政中心-通知公告-列表
    /// </summary>
    ...
    public partial class NoticeList : BackPage
    {
        ...
        private void PowerControl()
        {
            ...
            if (!this.CheckGrant(EyouSoft.Model.EnumType.PrivsStructure.Privs.行政中心_通知公告_新增))
            {
                ph_Add.Visible = false;
            }
            if (!this.CheckGrant(EyouSoft.Model.EnumType.PrivsStructure.Privs.行政中心_通知公告_修改))
            {
                ph_Update.Visible = false;
            }
            if (!this.CheckGrant(EyouSoft.Model.EnumType.PrivsStructure.Privs.行政中心_通知公告_删除))
            {
