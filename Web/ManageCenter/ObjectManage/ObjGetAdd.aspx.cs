...
namespace Web.ManageCenter.ObjectManage
{
    ...
    /// 修改人：方琪
    /// 修改时间：2012-03-31
    public partial class ObjGetAdd : BackPage
    {
        ...
        protected void PageSave()
        {
            ...
            string gettime = Utils.GetFormValue(txtGetTime.UniqueID);
            ...
            for (int i = 0; i < id.Length; i++)
            {
                ...
                model.Type = EyouSoft.Model.EnumType.GovStructure.GoodUseType.领用;
                model.Use = use;
            ...
            msg = result == 0 ? "物品领用成功！" : result == -1 ? "物品领用失败！" : "领用物品数量超出库存数量!";
            Response.Clear();
            Response.Write(UtilsCommons.AjaxReturnJson(result.ToString(), msg));
            Response.End();
            #endregion
        ...
        protected void PowerControl()
        {
            ...
            if (!this.CheckGrant(EyouSoft.Model.EnumType.PrivsStructure.Privs.行政中心_物品管理_领用登记))
            {
                Utils.ResponseNoPermit(EyouSoft.Model.EnumType.PrivsStructure.Privs.行政中心_物品管理_领用登记, false);
            }
        }
        #endregion
