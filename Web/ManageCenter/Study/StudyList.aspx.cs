...
namespace EyouSoft.Web.ManageCenter.Study
{
    ...
    /// 学习交流
    /// </summary>
    public partial class StudyList : BackPage
    {
        #region 页面参数
        /// <summary>
        /// 页大小
        ...
        protected string GetAttach(object obj)
        {
            ...
            {
                return string.Format("<a target=\"_blank\" title=\"附件\" href=\"/CommonPage/FileDownLoad.aspx?doType=downLoad&filePath={0}&name={1}\" ><img src=\"/Images/fujian.gif\" /></a>", items[0].FilePath, items[0].Name);
            }
        ...
        {
            if (!this.CheckGrant(EyouSoft.Model.EnumType.PrivsStructure.Privs.行政中心_学习交流_栏目))
            {
                Utils.ResponseNoPermit(EyouSoft.Model.EnumType.PrivsStructure.Privs.行政中心_学习交流_栏目, false);
            }
            if (!this.CheckGrant(EyouSoft.Model.EnumType.PrivsStructure.Privs.行政中心_学习交流_新增))
            {
                ph_Add.Visible = false;
            }
            if (!this.CheckGrant(EyouSoft.Model.EnumType.PrivsStructure.Privs.行政中心_学习交流_修改))
            {
                ph_Update.Visible = false;
            }
            if (!this.CheckGrant(EyouSoft.Model.EnumType.PrivsStructure.Privs.行政中心_学习交流_删除))
            {
