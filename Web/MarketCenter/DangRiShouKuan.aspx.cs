...
namespace EyouSoft.Web.MarketCenter
{
    ...
    /// 邵权江 2012-04-18
    /// </summary>
    public partial class DangRiShouKuan : EyouSoft.Common.Page.BackPage
    {
        #region 分页参数
        /// <summary>
        /// 页记录数
        ...
        void PowerControl()
        {
            if (((Menu2)Utils.GetInt(Utils.GetQueryStringValue("sl"))) == Menu2.销售中心_销售收款)
            {
                if (!CheckGrant(EyouSoft.Model.EnumType.PrivsStructure.Privs.销售中心_销售收款_栏目))
                {
                    Utils.ResponseNoPermit(EyouSoft.Model.EnumType.PrivsStructure.Privs.销售中心_销售收款_查看当日收款, true);
                    return;
                }
            }
            if (((Menu2)Utils.GetInt(Utils.GetQueryStringValue("sl"))) == Menu2.财务管理_应收管理)
            {
                if (!CheckGrant(EyouSoft.Model.EnumType.PrivsStructure.Privs.财务管理_应收管理_查看当日收款))
                {
                    Utils.ResponseNoPermit(EyouSoft.Model.EnumType.PrivsStructure.Privs.财务管理_应收管理_查看当日收款, true);
                    return;
                }
            }
        }
