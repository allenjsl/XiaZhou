...
namespace Web.MasterPage
{
    public partial class Front : System.Web.UI.MasterPage
    {
        ...
        protected string LogoBind()
        {
            ...
            {
                s += " style=\"background:url(" + setting.NLogo + ")\"";

                setting = null;
            }
            else
            {
                s += " style=\"background:url(/Images/nologo.jpg)\"";
            }
        ...
        protected string MenuBind()
        {
            ...
            if (menuList != null && menuList.Count > 0)
            {
                ...
                for (int i = 0; i < menuList.Count; i++)
                {
                    ...
                    //{
                    string menuFrist = "<li><h5><s class=\"" + menuList[i].ClassName + "\"></s>" + menuList[i].Name + "</h5>{0}</li>";
                    //用来验证是否拥有一级栏目权限
                    bool isGrant = false;
                    string menuSecond = "";
                    //判断是否拥有二级栏目
                    if (menuList[i].Menu2s != null)
                    {
                        ...
                        for (int j = 0; j < menuList[i].Menu2s.Count; j++)
                        {
                            ...
                            if (backPage.SiteUserInfo.Privs.Contains(menuList[i].Menu2s[j].Menu2PrivId))
                            {
                                //拥有二级栏目权限时即显示一级栏目
                                isGrant = true;
                                menuSecond += "<a href=\"" + menuList[i].Menu2s[j].Url + "\">" + menuList[i].Menu2s[j].Name + "</a>";
                    ...
                    if (isGrant)
                    {
                        menuFrist = string.Format(menuFrist, menuSecond);
