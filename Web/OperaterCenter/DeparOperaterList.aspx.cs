...
namespace Web.OperaterCenter
{
    ...
    /// 创建时间：2011-09-07
    /// </summary>
    public partial class DeparOperaterList : Eyousoft.Common.Page.BackPage
    {
        #region 分页参数
        /// <summary>
        /// 每页显示条数(常量)
        ...
        private void DataInit()
        {
            ...
            IList<EyouSoft.Model.TourStructure.MPlanList> Deparlist = new EyouSoft.BLL.TourStructure.BTour().GetCJPlanList(SiteUserInfo.CompanyId, pageSize, pageIndex, ref recordCount, Search);
            if (Deparlist != null && Deparlist.Count > 0)
            {
                this.DeparList.DataSource = Deparlist;
                this.DeparList.DataBind();
                //绑定分页
                BindPage();
            }
            else
        ...
        protected string GetOperate(string state, string tourid)
        {
            ...
            {
                str = "<a href=\"/OperaterCenter/OperaterVisaList.aspx?sl=" + sl + "&type=Departure&Id=" + tourid + "\">安排</a>";           
            }
            return str;
        }
        #endregion
        ...
        {
            if (CheckGrant(Common.Enum.TravelPermission.计调中心_出境计调_栏目))
            {
                if (!CheckGrant(Common.Enum.TravelPermission.计调中心_出境计调_查看全部))
                {
                    Utils.ResponseNoPermit(Common.Enum.TravelPermission.计调中心_出境计调_查看全部, false);
                    return;
                }
            }
            else
            {
                Utils.ResponseNoPermit(Common.Enum.TravelPermission.计调中心_出境计调_栏目, false);
                return;
            }
        }
        #endregion
