<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PrepaidAppliaction.aspx.cs"
    Inherits="Web.OperaterCenter.PrepaidAppliaction" %>
                    ...
                    <ItemTemplate>
                        <tr>
                            ...
                            <td align="center">
                                ...
                                <asp:PlaceHolder runat="server" Visible='<%#GetFinStatus((EyouSoft.Model.EnumType.FinStructure.FinStatus)Enum.Parse(typeof(EyouSoft.Model.EnumType.FinStructure.FinStatus), Eval("Status").ToString())) %>'>
    ...
    <script type="text/javascript">
        var PrepaidPage = {
            ...
            _Save: function(obj, ID) {
                $.newAjax({
                    type: "POST",
                    url: "/OperaterCenter/PrepaidAppliaction.aspx?sl=" + PrepaidPage.sl + "&action=" + obj + "&PlanId=" + PrepaidPage.planID + "&tourId=" + PrepaidPage.tourID + "&ID=" + ID,
                    cache: false,
                    dataType: 'json',
                    ...
                    success: function(ret) {
                        if (ret.result == "1") {
                            tableToolbar._showMsg(ret.msg, function() {
                                window.location.href = '/OperaterCenter/PrepaidAppliaction.aspx?sl=' + PrepaidPage.sl + "&PlanId=" + PrepaidPage.planID + "&tourId=" + PrepaidPage.tourID + "&souceName=" + PrepaidPage.souceName + "&iframeId=" + PrepaidPage._iframeId;
                            });
                        } else {
                            tableToolbar._showMsg(ret.msg, function() {
            ...
            _Delete: function(_ObjID) {
                $.newAjax({
                    type: "get",
                    url: '/OperaterCenter/PrepaidAppliaction.aspx?sl=' + PrepaidPage.sl + '&action=delete&ID=' + _ObjID,
                    cache: false,
                    dataType: 'json',
                    success: function(ret) {
                        if (ret.result == "1") {
            ...
            _BindBtn: function() {
                ...
                $("#tabPrepaidList").find("[data-class='update']").click(function() {
                    ...
                    if (id) {
                        window.location.href = '/OperaterCenter/PrepaidAppliaction.aspx?type=' + PrepaidPage.type + '&sl=' + PrepaidPage.sl + '&tourID=' + PrepaidPage.tourID + '&action=update&ID=' + id + "&PlanId=" + PrepaidPage.planID + "&souceName=" + escape(PrepaidPage.souceName) + "&iframeId=" + PrepaidPage._iframeId;
                    }
                });
