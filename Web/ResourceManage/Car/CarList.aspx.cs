...
namespace Web.ResourceManage.Car
{
    ...
    /// 说明: 资源管理： 车队
    /// 
    public partial class CarList : BackPage
    {
        #region attributes
        /// <summary>
        /// 每页显示条数
        ...
        void InitRpt()
        {
            ...
            var items = new EyouSoft.BLL.GysStructure.BGys().GetCheDuis(SiteUserInfo.CompanyId, pageSize, pageIndex, ref recordCount, chaXun);
            ...
            {
                NodataMsg = "<tr class='old'><td colspan='6' align='center'>没有相关数据</td></tr>";
        ...
        {
            if (!this.CheckGrant(EyouSoft.Model.EnumType.PrivsStructure.Privs.资源管理_车队_栏目))
            {
                Utils.ResponseNoPermit(EyouSoft.Model.EnumType.PrivsStructure.Privs.资源管理_车队_栏目, false);
                return;
            }
            ...
            if (!this.CheckGrant(EyouSoft.Model.EnumType.PrivsStructure.Privs.资源管理_车队_删除))
            {
        ...
        void ToXls()
        {
            ...
            int _recordCount = 0;
            var chaXun = GetChaXunInfo();
            var items = new EyouSoft.BLL.GysStructure.BGys().GetCheDuis(SiteUserInfo.CompanyId, toXlsRecordCount, 1, ref _recordCount, chaXun);
            if (items == null || items.Count == 0) ResponseToXls(string.Empty);


            var s = new StringBuilder();
            s.Append("所在地\t车队名称\t联系人\t交易次数\t车队数量\t结算金额\t未付金额\t质检均分\n");
        ...
        EyouSoft.Model.GysStructure.MLBChaXunInfo GetChaXunInfo()
        {
            ...
            info.GysName = Utils.GetQueryStringValue("txtteamName");
            info.CheDuiCheXingName = Utils.GetQueryStringValue("txtmodel");
