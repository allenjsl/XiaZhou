...
namespace Web.ResourceManage.Car
{
    ...
    public partial class Transactions : BackPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //导出处理
            if (UtilsCommons.IsToXls()) ListToExcel(sourceid);
            if (!IsPostBack)
            {
                //初始化
                DataInit(sourceid);
        ...
        private void DataInit(string id)
        {
            ...
            //IList<EyouSoft.Model.SourceStructure.MMotorcadeClosingCostModel> list = bll.GetMotorcadeTradeListModel(id, this.SiteUserInfo.CompanyId, pageIndex, pageSize, ref recordCount);
        ...
        private void ListToExcel(string id)
        {
            ...
            //团号 	线路名称 	用车时间 	销售员 	计调员 	带团导游 	明细 	结算金额 	未付金额
            s.Append("团号\t线路名称\t用车时间\t销售员\t计调\t导游\t明细\t结算金额\t未付金额\n");
            pageIndex = Utils.GetInt(Utils.GetQueryStringValue("page"));
            EyouSoft.BLL.SourceStructure.BSource bll = new EyouSoft.BLL.SourceStructure.BSource();
            //IList<EyouSoft.Model.SourceStructure.MMotorcadeClosingCostModel> list = bll.GetMotorcadeTradeListModel(id, this.SiteUserInfo.CompanyId, pageIndex, toXlsRecordCount, ref recordCount);
            //if (list != null && list.Count > 0)
            //{
            ...
            //            t.TourCode,
            //            t.RouteName,
            //            UtilsCommons.GetDateString(t.UseTime, ProviderToDate),
            //            t.Seller,
