...
namespace EyouSoft.Web.ResourceManage
{
    ...
    /// 供应商交易明细
    /// </summary>
    public partial class JiaoYiMingXi : BackPage
    {
        #region attributes
        /// <summary>
        /// 每页显示条数
        ...
        EyouSoft.Model.GysStructure.MJiaoYiMingXiChaXunInfo GetChaXunInfo()
        {
            ...
            if (GysId == "all") info.GysLeiXing = (EyouSoft.Model.EnumType.SourceStructure.SourceType?)Utils.GetEnumValueNull(typeof(EyouSoft.Model.EnumType.SourceStructure.SourceType), Utils.GetQueryStringValue("gysleixing"));
        ...
        void ToXls()
        {
            ...
            if (toXlsRecordCount >= 2000) Utils.RCWE("当前导出数据量较大(一次最多可导出2000个交易明细)，请分批次导出。<a href=\"javascript:history.go(-1)\">点击这里返回</a>");
            ...
            s.Append("团号\t线路名称\t销售员\t计调\t导游\t供应商\t数量\t费用明细\t结算金额\t未付金额\t支付方式\n");
            ...
            foreach (var item in items)
            {
                ...
                s.Append(item.FeiYongMingXi.Replace("\t", "    ").Replace("\r\n", "    ").Replace("\r", "    ").Replace("\n", "    ") + "\t");
                s.Append(item.JinE.ToString("F2") + "\t");
                s.Append(item.WeiZhiFuJinE.ToString("F2") + "\t");
                s.Append(item.ZhiFuFangShi + "\n");
            }
