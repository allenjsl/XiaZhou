<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/Front.Master" AutoEventWireup="true"
    CodeBehind="PathList.aspx.cs" Inherits="Web.ResourceManage.Path.PathList" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div class="mainbox">
        <div class="searchbox fixed">
            ...
            <span class="searchT">
                <p>
                    线路区域：<select name="PathType" id="PathType" class="inputselect">
                        <%=EyouSoft.Common.UtilsCommons.GetAreaLineForSelect(EyouSoft.Common.Utils.GetInt(EyouSoft.Common.Utils.GetQueryStringValue("PathType")),this.SiteUserInfo.CompanyId)%>
                    </select>
                    线路名称：
                    <input type="text" size="28" maxlength="30" class="inputtext" name="txtPathName"
                        value='<%=Request.QueryString["txtPathName"]%>' />
                    天数：
                    <input type="text" size="10" onafterpaste="this.value=this.value.replace(/\D/g,'')"
                        onkeyup="this.value=this.value.replace(/\D/g,'')" class="inputtext" name="txtData"
                        value='<%=Request.QueryString["txtData"]%>' valid="isNumber" errmsg="必须是数字!" />
                    发布人：
                        ...
                        <ItemTemplate>
                            <tr class="<%#Container.ItemIndex%2==0 ? "odd":"" %>">
                                <td align="center">
                                    ...
                                    <input type="checkbox" id="checkbox" name="checkbox" value="<%#Eval(" RouteID") %>">
                                </td>
                                <td align="center">
                                    <%#Eval("RouteTypeName")%>
                                </td>
                                <td align="left">
                                    <a target="_blank" href="<%#PrintPageXL %>?xianluid=<%#Eval("RouteID") %>" data-id='<%#Eval("RouteID") %>'>
                                        <%#Eval("RouteName")%></a>
                                </td>
                                <td align="center">
                                    <%#Eval("DayCount")%>
                                ...
                                <td align="center">
                                    <span style="display: none">
                                        <%#GetContactInfo(Eval("UserList"))%></span> <a data-contact="contact" href="javascript:void(0)"
                                            class="contact" title="">
                                            <%#Eval("OperatorName")%></a>
                                </td>
                                <td align="center">
                                    <a href="javascript:void(0)" data-sklist="SKList" data-rel="<%#Eval("RouteID") %>"><b
                                        class="fontgreen">
                                        <%#Eval("STCount")%></b></a>
                                </td>
                                <td align="center">
                                    <b class="fontred"><a href="avascript:void(0)" data-goustlist="GoustList" data-rel="<%#Eval("RouteID") %>">
                                        <%#Eval("SKCount")%></a></b>
                                </td>
                            </tr>
                        </ItemTemplate>
    ...
    <script type="text/javascript">
        ...
        $(function() {
            ...
            $("#tablelist").find("a[Data-SKList='SKList']").click(function() {
                var url = "/ResourceManage/Path/TourCountList.aspx?routeid=" + $(this).attr("Data-rel") + "&sl=" + '<%=Request.QueryString["sl"] %>';
                Boxy.iframeDialog({
                    iframeUrl: url,
                    title: "上团数",
                    modal: true,
            ...
            $("#tablelist").find("a[Data-GoustList='GoustList']").click(function() {
                var url = "/ResourceManage/Path/GoustList.aspx?routeid=" + $(this).attr("Data-rel") + "&sl=" + '<%=Request.QueryString["sl"] %>';
                Boxy.iframeDialog({
                    iframeUrl: url,
                    title: "收客数",
                    modal: true,
            ...
            $(".a_UnitName").click(function() {
                ...
                var self = this;
                url += $.param({
                    sl: '<%=(int)EyouSoft.Model.EnumType.PrivsStructure.Menu2.资源管理_线路资源 %>',
                    chakan: "chakan",
                    dotype: "update",
        ...
        var PathListPage = {
            Add: function() {
                window.location.href = "/ResourceManage/Path/PathEidt.aspx?doType=add&sl=<%=(int)EyouSoft.Model.EnumType.PrivsStructure.Menu2.资源管理_线路资源 %>";
            },
            Update: function(objsArr) {
                window.location.href = "/ResourceManage/Path/PathEidt.aspx?doType=update&sl=<%=(int)EyouSoft.Model.EnumType.PrivsStructure.Menu2.资源管理_线路资源 %>&id=" + objsArr[0].find("input[type='checkbox']").val();
            },
            Copy: function(objsArr) {
                window.location.href = "/ResourceManage/Path/PathEidt.aspx?doType=copy&sl=<%=(int)EyouSoft.Model.EnumType.PrivsStructure.Menu2.资源管理_线路资源 %>&id=" + objsArr[0].find("input[type='checkbox']").val();
            },
            DelAll: function(objArr) {
                //ajax执行文件路径,默认为本页面
                var ajaxUrl = "/ResourceManage/Path/PathList.aspx";
                //定义数组对象
                var list = new Array();
                //遍历按钮返回数组对象
                ...
                ajaxUrl += "?doType=delete&sl=<%=(int)EyouSoft.Model.EnumType.PrivsStructure.Menu2.资源管理_线路资源 %>&id=" + list.join(',');
                //执行ajax
