...
namespace Web.ResourceManage.scenic_spots
{
    ...
    public partial class Transactions : BackPage
    {
        ...
        private void DataInit(string id)
        {
            ...
            //IList<EyouSoft.Model.SourceStructure.MSpotClosingCosttModel> list = bll.GetSpotTradeListModel(id, this.SiteUserInfo.CompanyId, this.pageIndex, pageSize, ref recordCount);
        ...
        private void ListToExcel(string id)
        {
            ...
            s.Append("团号\t线路名称\t人数\t销售员\t计调\t带团导游\t明细\t结算金额\t未付金额\n");
            pageIndex = Utils.GetInt(Utils.GetQueryStringValue("page"));
            EyouSoft.BLL.SourceStructure.BSource bll = new EyouSoft.BLL.SourceStructure.BSource();
            //IList<EyouSoft.Model.SourceStructure.MSpotClosingCosttModel> list = bll.GetSpotTradeListModel(id, this.SiteUserInfo.CompanyId, this.pageIndex, toXlsRecordCount, ref recordCount);
            //if (list != null && list.Count > 0)
            //{
            ...
            //            t.TourCode,
            //            t.RouteName,
            //            (t.PeopleCountModel.AdultCount.ToString()) + "+" + (t.PeopleCountModel.ChildrenCount.ToString()) + "+" + (t.PeopleCountModel.OtherCount.ToString()),
            //            t.Seller,
