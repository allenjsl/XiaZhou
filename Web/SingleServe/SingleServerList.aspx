<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/Front.Master" AutoEventWireup="true"
    CodeBehind="SingleServerList.aspx.cs" Inherits="Web.SingleServe.SingleServerList" %>
            ...
            <span class="searchT">
                <p>
                    ...
                    <select name="status" id="status" class="inputselect">
                        ...
                        <option value="0">操作中</option>
                        <option value="3">已落实</option>
                        <option value="9">待终审</option>
                        <option value="10">财务核算</option>
                        <option value="11">核算结束</option>
                        <option value="12">已取消</option>
                    </select>
                    <br />
                    委托日期：<input type="text" onfocus="WdatePicker()" class="inputtext formsize80" name="txtSWeiTuoRiQi"
                        value='<%=Request.QueryString["txtSWeiTuoRiQi"]%>' />-<input type="text" onfocus="WdatePicker()" class="inputtext formsize80" name="txtEWeiTuoRiQi"
                        value='<%=Request.QueryString["txtEWeiTuoRiQi"]%>' />
                    &nbsp; 销售员：<uc1:SellsSelect ID="txtXiaoShouYuan" runat="server" SelectFrist="false" />
                    &nbsp;计调员： <uc1:SellsSelect ID="txtJiDiaoYuan" runat="server" SelectFrist="false" />
                    &nbsp;<input type="hidden" name="sl" value="<%=Request.QueryString["sl"] %>" /><input
                        type="submit" value="搜索" class="search-btn" /></p>
            </span>
        </div>
        </form>
                    ...
                    <asp:Repeater ID="rptList" runat="server">
                        <ItemTemplate>
                            <tr <%#Container.ItemIndex%2==0?" class=\"odd\" ":""  %> i_tourid="<%#Eval("TourId") %>"
                                data-dingdanid="<%#Eval("OrderId") %>">
                                <td align="center">
                                    <input type="hidden" name="ItemUserID" value="<%#Eval("OperatorId")%>" />
                                    <input type="checkbox" id="checkbox" name="checkbox" value="<%#Eval("TourId")%>">
                                </td>
                                <td align="center">
                                    <%#Eval("WeiTuoRiQi","{0:yyyy-MM-dd}")%>
                                </td>
                                <td align="center">
                                    <a href="javascript:void(0)" class="i_danju"><%#Eval("OrderCode")%></a>
                                </td>
                                <td align="left">
                                    <a href="javascript:void(0)" class="i_hetong"><%#GetHeTongCode(Eval("HeTongCode"))%></a>
                                </td>
                                <td align="left">
                                    <%#Eval("BuyCompanyName")%>
                                </td>
                                ...
                                <td align="center">
                                    <%#Eval("PlanProjectType")%>
                                </td>
                                <td>
                                    <%#Eval("Operator")%>
                                </td>
                                <td align="center">
                                    <%#GetTourStatus(Eval("TourStatus"), Eval("QuXiaoYuanYin"))%>
                                </td>
                            </tr>
                        </ItemTemplate>
                    </asp:Repeater>
    ...
    <script type="text/javascript">
        var SinglePage = {
            ...
            Add: function() {
                ...
                data.title = "新增单项业务";
                data.url = "/SingleServe/SingleServerEdit.aspx?sl=<%=(int)EyouSoft.Model.EnumType.PrivsStructure.Menu2.单项业务_单项业务 %>";
                data.width = "985px";
                data.height = "500px";
            ...
            Update: function(objsArr) {
                var data = SinglePage.dataBoxy();
                data.title = "修改单项业务";
                data.url = "/SingleServe/SingleServerEdit.aspx?sl=<%=(int)EyouSoft.Model.EnumType.PrivsStructure.Menu2.单项业务_单项业务 %>&doType=update&id=" + objsArr[0].find("input[type='checkbox']").val();
                data.width = "985px";
                data.height = "500px";
                SinglePage.ShowBoxy(data);
            },
            ...
            danJu: function(obj) {
                ...
                var _data = { sl: '<%=EyouSoft.Common.Utils.GetQueryStringValue("sl") %>', tourid: _$tr.attr("i_tourid") };
                Boxy.iframeDialog({ iframeUrl: "danxiangyewudanju.aspx", title: "单项服务相关单据", modal: true, width: "240px", height: "300px", data: _data });
            },
            quXiao: function(obj) {
            ...
            sheZhiHeTong: function(obj) {
                var _$tr = $(obj).closest("tr");
                var _dingDanId = _$tr.attr("data-dingdanid");
                var _data = { orderid: _dingDanId,sl:"<%=SL %>" };
                Boxy.iframeDialog({ title: "设置合同", iframeUrl: "/SellCenter/Order/SetContract.aspx", width: "600px", height: "300px", data: _data, afterHide: function() { SinglePage.reload(); } });
                return false;
            }
        }
        ...
        $(function() {
            ...
            $(".i_danju").click(function() { SinglePage.danJu(this); });
            ...
            $(".i_hetong").click(function() { SinglePage.sheZhiHeTong(this); });
        })
    </script>
