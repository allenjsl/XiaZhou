...
namespace Web.SystemSet
{
    ...
    /// 修改记录：
    /// 1、2011-10-9 曹胡生 创建
    public partial class GuestLevelList : BackPage
    {
        protected int i = 0;
        ...
        private void DelStard()
        {
            ...
            {
                EyouSoft.Common.Function.MessageBox.ResponseScript(this, string.Format("tableToolbar._showMsg('删除成功');window.location.href='GuestLevelList.aspx?sl={0}&memuid={1}';", SL, memuid));
            }
            else
            {
                EyouSoft.Common.Function.MessageBox.ResponseScript(this, string.Format("tableToolbar._showMsg('删除失败');window.location.href='GuestLevelList.aspx?sl={0}&memuid={1}';", SL, memuid));
            }
        }
