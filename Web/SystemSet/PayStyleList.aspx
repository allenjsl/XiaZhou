<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/Front.Master" AutoEventWireup="true"
    CodeBehind="PayStyleList.aspx.cs" Inherits="Web.SystemSet.PayStyleList" %>
                    ...
                    <cc1:CustomRepeater ID="repList" runat="server">
                        <ItemTemplate>
                            <tr class="<%#Container.ItemIndex%2==0?"":"#odd" %>">
                                ...
                                <td align="center">
                                    <a class="update_zffs" href="PayStyleEdit.aspx?id=<%#Eval("PaymentId") %>&sl=<%=SL%>&memuid=<%=memuid%>">
                                        <%#Convert.ToBoolean(Eval("IsSystem")) ? "":"修改&nbsp;|"%></a><a href="PayStyleList.aspx?ids=<%#Eval("PaymentId") %>&sl=<%=SL %>&state=del&memuid=<%=memuid %>" onclick=" return PayStyleList.Del(this);">
    ...
    <script type="text/javascript">
        ...
        $(function() {
            $(".add_xlqy").click(function() {
                var url = "PayStyleEdit.aspx?sl=<%=SL%>&memuid=<%=memuid%>";
                Boxy.iframeDialog({
                    iframeUrl: url,
                    title: "添加",
                    modal: true,
