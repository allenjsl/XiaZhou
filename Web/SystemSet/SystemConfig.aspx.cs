...
namespace Web.SystemSet
{
    ...
    /// 修改记录：
    /// 1、2011-10-9 曹胡生 创建
    public partial class SystemConfig : BackPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //this.UploadOutLogo.CompanyID = SiteUserInfo.CompanyId;
            //this.UploadOutLogo.IsUploadMore = false;
            //this.UploadOutLogo.IsUploadSelf = true;
            //this.UploadOutLogo.FileTypes = "*.jpg;*.gif;*.jpeg;*.png";
            //this.UploadInnerLogo.CompanyID = SiteUserInfo.CompanyId;
            //this.UploadInnerLogo.IsUploadMore = false;
            //this.UploadInnerLogo.IsUploadSelf = true;
            //this.UploadInnerLogo.FileTypes = "*.jpg;*.gif;*.jpeg;*.png";

            UploadOutLogo.CompanyID = UploadInnerLogo.CompanyID = ucMLogo.CompanyID = ucGYSLogo.CompanyID = ucFXSLogo.CompanyID = SiteUserInfo.CompanyId;
            UploadOutLogo.IsUploadSelf = UploadInnerLogo.IsUploadSelf = ucMLogo.IsUploadSelf = ucGYSLogo.IsUploadSelf = ucFXSLogo.IsUploadSelf = true;
            UploadOutLogo.FileTypes = UploadInnerLogo.FileTypes = ucMLogo.FileTypes = ucGYSLogo.FileTypes = ucFXSLogo.FileTypes = "*.jpg;*.gif;*.jpeg;*.png";
        ...
        protected void PageInit()
        {
            ...
            if (model != null)
            {
                ...
                if (!string.IsNullOrEmpty(model.MLogo))
                {
                    lblMLogo.Text = string.Format("<span class='upload_filename'>&nbsp;<a href='{0}' target='_blank'>查看</a><a href='javascript:void(0);' onclick='SystemConfig.RemoveFile(this);return false;'> <img style='vertical-align:middle' src='/images/cha.gif'></a><input type='hidden' name='txtMLogo' value='{0}'/></span>", model.MLogo);
                }
                if (!string.IsNullOrEmpty(model.GYSLogo))
                {
                    lblGYSLogo.Text = string.Format("<span class='upload_filename'>&nbsp;<a href='{0}' target='_blank'>查看</a><a href='javascript:void(0);' onclick='SystemConfig.RemoveFile(this);return false;'> <img style='vertical-align:middle' src='/images/cha.gif'></a><input type='hidden' name='txtGYSLogo' value='{0}'/></span>", model.GYSLogo);
                }
        ...
        protected void Save()
        {
            ...
            string mLogo= Utils.GetFormValue(ucMLogo.ClientHideID);
            string gysLogo = Utils.GetFormValue(ucGYSLogo.ClientHideID);
            ...
            if (string.IsNullOrEmpty(mLogo))
            {
                model.MLogo = Utils.GetFormValue("txtMLogo");
            }
            else
            {
                model.MLogo = mLogo.Split('|')[1];
            }

            if (string.IsNullOrEmpty(gysLogo))
            {
                model.GYSLogo = Utils.GetFormValue("txtGYSLogo");
            }
            else
            {
                model.GYSLogo = gysLogo.Split('|')[1];
            }
