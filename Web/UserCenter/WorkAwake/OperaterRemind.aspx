                    ...
                    <asp:Repeater ID="rptList" runat="server">
                        <ItemTemplate>
                            <tr class="<%#Container.ItemIndex%2==0?"":"odd" %>">
                                ...
                                <td align="center">
                                    <%#EyouSoft.Common.Utils.GetDateTime(Eval("LDate").ToString()).ToShortDateString()%>
                                </td>
                                <td align="center">
    ...
    <script type="text/javascript">
        var OperaterRemind = {
                ...
                _OperaterReceive: function(comID, tourId,type) {
                    $.newAjax({
                        type: "POST",
                        url: '/Ashx/ReceiveJob.ashx?sl=<%=EyouSoft.Common.Utils.GetQueryStringValue("sl") %>&type=receive&com=' + comID + "&Operator=<%=this.SiteUserInfo.Username %>" + "&OperatorID=<%=this.SiteUserInfo.UserId %>" + "&OperatDepID=<%=this.SiteUserInfo.DeptId  %>&tourId=" + tourId,
                        async: false,
                        dataType: "json",
                        success: function(data) {
                            if (data.result) {
                                tableToolbar._showMsg(data.msg, function() {
                                    if(type=='<%=EyouSoft.Model.EnumType.TourStructure.TourType.地接散拼 %>'||
                                       type=='<%=EyouSoft.Model.EnumType.TourStructure.TourType.组团散拼 %>'||
                                       type=='<%=EyouSoft.Model.EnumType.TourStructure.TourType.出境散拼 %>'){
                                        window.location.href = "/OperaterCenter/OperaterConfigPage.aspx?sl=<%=EyouSoft.Common.Utils.GetQueryStringValue("sl") %>&type=OrderRemind&tourId=" + tourId + "&tourType=sanping";
                                    }
                                    else if(type=='<%=EyouSoft.Model.EnumType.TourStructure.TourType.出境团队 %>'||
                                            type=='<%=EyouSoft.Model.EnumType.TourStructure.TourType.地接团队 %>'||
                                            type=='<%=EyouSoft.Model.EnumType.TourStructure.TourType.组团团队 %>'||
                                            type=='<%=EyouSoft.Model.EnumType.TourStructure.TourType.单项服务 %>'){
                                        window.location.href = "/OperaterCenter/OperaterConfigPage.aspx?sl=<%=EyouSoft.Common.Utils.GetQueryStringValue("sl") %>&type=OrderRemind&tourId=" + tourId + "&tourType=zutuan";
                                   }
                                });
