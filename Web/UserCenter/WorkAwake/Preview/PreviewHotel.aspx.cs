...
{
    public partial class PreviewHotel : BackPage
    {
        /// <summary>
        /// 页面：DOM
        /// </summary>
        ...
        private void DataInit()
        {
            ...
            IList<MSourceSueHotel> list = bllBIndividual.GetHotelcontrolHotelUsedLst(pageSize, pageIndex, ref recordCount, SiteUserInfo.CompanyId, SiteUserInfo.UserId);
        ...
        private string SetLastTime(string day)
        {
            string msg = string.Empty;
            //设置操作
            if (new EyouSoft.BLL.ComStructure.BComSetting().SetJiuDianYuKongTiXing(SiteUserInfo.CompanyId, Utils.GetInt(day)))
            {
                msg = UtilsCommons.AjaxReturnJson("true", "您设置酒店预控到期前" + day + "提醒成功");
            }
            else
                msg = UtilsCommons.AjaxReturnJson("false", "您设置酒店预控到期前" + day + "提醒失败");
        ...
        protected string GetLastTime()
        {
            ...
            if (setting != null) return setting.HotelControlRemind.ToString();
