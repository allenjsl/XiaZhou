...
{
    public partial class OrderPay : WzPage
    {
        ...
        void InitHangQiDingDan()
        {
            ...
            if (info.DingDanStatus != EyouSoft.Model.EnumType.YlStructure.HangQiDingDanStatus.成交) RCWE("你不能支付该订单");
            if (info.FuKuanStatus == EyouSoft.Model.EnumType.YlStructure.FuKuanStatus.已付款) RCWE("该订单已支付");
            ...
            if (info.XiaDanRenId != Utils.GetQueryStringValue("token")) RCWE("你不能支付该订单");
            if (isLogin && info.XiaDanRenId != huiYuanInfo.HuiYuanId)RCWE("你不能支付该订单");
        ...
        void InitDuiHuanDingDan()
        {
            ...
            if (info == null) RCWE("异常请求");
            if (info.DingDanStatus != EyouSoft.Model.EnumType.YlStructure.JiFenDingDanStatus.已成交) RCWE("你不能支付该订单");
            if (info.FuKuanStatus == EyouSoft.Model.EnumType.YlStructure.FuKuanStatus.已付款) RCWE("该订单已支付");
        ...
        void Send()
        {
            ...
            else if (bankinfo.fs == "1")
            {
                ...
                if (bankinfo.bankid == "99BILL_KJ")
                {
                    url = "/99bill/send.aspx?bank_kj=1&dingdanid=" + dingdanid + "&dingdanleixing=" + dingdanleixing + "&token=" + token;
                }
            }
    ...
    public class MZaiXianZhiFuFangShiInfo
    {
        ...
        public IList<MZaiXianZhiFuFangShiInfo> GetZaiXianZhiFuFangShis()
        {
            ...
            //items.Add(new MZaiXianZhiFuFangShiInfo() { id = "1B2589E5-6CAB-463C-9924-45D77459B860", name = "快钱快捷支付", bankid = "99BILL_KJ", fs = "1", logo = "/images/banklogo/bank_99bill_kj.jpg" });
