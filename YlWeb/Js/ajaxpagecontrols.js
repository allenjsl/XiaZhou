...
AjaxPageControls.createPrev = function() {
    if (this.config.pageIndex > 1) {
        ...
        return '<li class="prev"><a href="javascript:void(0)" onclick="' + this.config.gotoPageFunctionName + '(' + prevPageIndex + ')">上一页</a></li>';
    } else {
        if (!this.config.showSY) return '<li class="prev disabled">上一页</li>';
    }
    return '';
};
...
AjaxPageControls.createPage = function(pageIndex) {
    ...
    if (pageIndex != this.config.pageIndex) {
        s = '<li class="list"><a href="javascript:void(0)" onclick="' + this.config.gotoPageFunctionName + '(' + pageIndex + ')">' + pageIndex + '</a></li>';
    } else {
        s = '<li class="current">' + pageIndex + '</li>';
    }
...
AjaxPageControls.createNext = function() {
    if (this.config.pageIndex != this.config.pageCount) {
        ...
        return '<li class="next"><a href="javascript:void(0)" onclick="' + this.config.gotoPageFunctionName + '(' + nextPageIndex + ')">下一页</a></li>';
    } else {
        if (!this.config.showWY) return '<li class="next disabled">下一页</li>';
    }
    return '';
}

AjaxPageControls.createSY = function() {
    if (this.config.pageIndex != 1) {
        return '<li class="sy"><a href="javascript:void(0)" onclick="' + this.config.gotoPageFunctionName + '(' + 1 + ')">首页</a></li>';
    } else {
        return '<li class="sy disabled">首页</li>';
    }
}

AjaxPageControls.createWY = function() {
    if (this.config.pageIndex != this.config.pageCount) {
        return '<li class="wy"><a href="javascript:void(0)" onclick="' + this.config.gotoPageFunctionName + '(' + this.config.pageCount + ')">尾页</a></li>';
    } else {
        return '<li class="wy disabled">尾页</li>';
    }
}
...
AjaxPageControls.createHTML = function(elementId) {
    ...
    if (this.config.showSY) {
        s.push(this.createSY());
    }
    ...
    */
    ...
    if (this.config.showWY) {
        s.push(this.createWY());
    }
