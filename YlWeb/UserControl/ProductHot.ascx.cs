...
{
    public partial class ProductHot : System.Web.UI.UserControl
    {
            ...
            if (obj != null)
            {
                ...
                if (list != null && list.Count > 0)
                {
                    if (type.ToString() == "top")
                    {
                        ...
                        sb.AppendFormat("<li {0}><img {2} src=\"{1}\" id='img_remen_{3}'></li>", view, EyouSoft.YlWeb.TuPian.F1(Utils.GetErpFilepath() + list[0].Filepath,450,278), width,hangQiId.ToString());
                    }
                    else
                    {
                        for (int i = 0; i < list.Count; i++)
                        {
                            sb.AppendFormat("<li {0}><a href='javascript:void(0)'><img {2} class=\"i_remen_img_2\" src=\"{1}\" data-src='{3}'></a></li>", view, EyouSoft.YlWeb.TuPian.F1(Utils.GetErpFilepath() + list[i].Filepath, 106, 76), width, EyouSoft.YlWeb.TuPian.F1(Utils.GetErpFilepath() + list[i].Filepath, 450, 278));
                        }
                    }
                }
            }
