...
{
    public partial class YouLunGongSi : WzPage
    {
        protected string GoSiName = string.Empty;
        ...
        void initPage()
        {
            ...
            if (model != null)
            {
                if (model.LeiXing == EyouSoft.Model.EnumType.YlStructure.YouLunLeiXing.长江游轮) Response.Redirect(string.Format("/youlun/YouLunGongSiCJ.aspx?id={0}", model.GongSiId));
        ...
        protected string getYLbyXL(string gongsi, string xilie)
        {
            ...
            if (items != null && items.Count > 0)
            {
                ...
                {
                    strbu.AppendFormat("<li><div class=\"div_img\"><a href=\"/Youlun/ChuanZhi.aspx?id={1}\"><img src=\"{0}\" style='width:161px;height:122px' /></a></div>", (items[i].FuJians != null && items[i].FuJians.Count > 0 ? TuPian.F1(ErpFilepath+items[i].FuJians[0].Filepath,161,122) : ""), items[i].ChuanZhiId);
                    strbu.AppendFormat("<dl><dt><a title='{0}' href=\"/Youlun/ChuanZhi.aspx?id={1}\">{0}</a></dt>", items[i].MingCheng, items[i].ChuanZhiId);
        ...
        private void GongSi()
        {
            ...
            var list = bll.GetGongSis(YuMingInfo.CompanyId, 10, 1, ref recordCount, new MGongSiChaXunInfo() { GongSiLeiXing = EyouSoft.Model.EnumType.YlStructure.YouLunLeiXing.海洋邮轮 });
